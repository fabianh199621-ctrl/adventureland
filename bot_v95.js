...
var KEEP_ITEMS = /^(hpot|mpot|elixirint|scroll|cscroll|intscroll|strscroll|dexscroll|vitscroll|tracker|seashell|monstertoken)/;
        ...
        while (rounds++ < 6) {
            ...
            for (var pack in bank) {
                ...
                for (var i = 0; i < bank[pack].length && character.esize > 2; i++) {
                    var it = bank[pack][i]; if (!it || worth_keeping(it) || equipped_names()[it.name]) continue;
...
async function kiss_routine() {
    ...
    try {
        ...
        if (ok) { kiss_mark_done(round); game_log("Kuss belohnt (Runde " + round + ")"); }
...
function hunt_target_ok(id) { return id && G.monsters[id] && is_safe_monster(id) && !hidden_mons[id] && !EXCLUDE[id]; }
...
async function check_monsterhunt() {
    ...
    try {
        ...
        await sleep(800);
        var q2 = mh_quest();
        if (q && q.c == 0) game_log("Monster Hunt abgegeben: +" + (tokens() - before_tok) + " Tokens, +" + fmt(character.gold - before_gold) + " Gold (" + tokens() + " Tokens gesamt)");
...
function start_main() {
  ...
  main_timer = parent.__lp_main_timer = setInterval(function () {
    ...
    run_goal(); check_weapon(); check_gear_slots(); check_flee(); check_elixir(); kiss_routine(); tidy_inventory(); check_potions(); check_stuck(); check_ponty(false); check_market(false); check_seashells(); check_monsterhunt();
    ...
    if (!target) {
        ...
        if (!target && bycatch && aggro < MAX_AGGRO) { // Beifang: nächstes sicheres Monster in der Nähe
            ...
            for (var bid in parent.entities) { var bm = parent.entities[bid]; if (!bm || bm.type != "monster" || bm.dead || bm.mtype == farm) continue; if (!is_safe_monster(bm.mtype) || hidden_mons[bm.mtype]) continue; if (bm.target && bm.target != character.name) continue; var dd = distance(character, bm); if (dd < bd) { bd = dd; target = bm; } }
        }

...
function update_panel() {
    ...
    h += "<div class='lp_row' style='flex-wrap:wrap;line-height:1.6'><span class='lp_k'>Alle Server:</span> <span style='font-size:11px'>" + global_html() + "</span></div>";
...
function wish_ui_html() { // Zielbau-Tabelle
    var h = "<table class='lp_t'><tr><th style='text-align:left'>Slot</th><th style='text-align:left'>Zielitem (Tooltip = Werte)</th><th>Stufe</th><th style='text-align:left'>Stand</th><th style='text-align:left'>Angebote alle Server (★ = hier)</th><th></th></tr>";
    for (var slot in SLOT_TYPES) {
        ...
        h += "<tr><td style='text-align:left'>" + slot + "</td><td style='text-align:left'>" + sel + "</td><td>" + lv + "</td><td style='text-align:left'>" + stand + "</td><td style='text-align:left;white-space:normal;max-width:190px'>" + offers_html(slot) + "</td><td><button data-act='focus' data-slot='" + slot + "' title='diesen Slot jetzt angehen'>Jetzt</button></td></tr>";
    }
    return h + "</table>";
}
...
function wish_wants(name, level, price) { // Slot, für den dieses Angebot die Wunschliste verbessert – sonst null
    ...
    for (var slot in WISHLIST) {
        ...
        var cap = WISH_MAX[name] || def.g * 8; if (price > cap || price > wish_budget()) continue;
        ...
        if (r > wr) return slot;
        if (r == wr && worn && (level || 0) >= (worn.level || 0) + 2 && price <= cap / 2) return slot; // gleiches Teil, deutlich höher, günstig
    }
    return null;
}
...
async function focus_slot(slot) { // Button "Jetzt": diesen Slot sofort verbessern – Wunschliste am Markt/Ponty, sonst getragenes Teil ausbauen
    ...
    try {
        ...
        if (worn && G.items[worn.name].compound) { check_pause(); await compound_slot(slot); await compound_spares(); await ensure_best_equipped(); }
        else if (worn && G.items[worn.name].upgrade) { check_pause(); await process_slot(slot, slot_target(slot, is_buyable(worn.name) ? UPGRADE_TARGET : (slot == "mainhand" ? WEAPON_SAFE_TARGET : SAFE_TARGET_DROP), 0)); }
        worn = character.slots[slot];
...
function offer_txt(o) { return o.name + "+" + o.level + " " + fmt(o.price) + " (" + o.server + (o.same ? " ★" : "") + ", " + o.seller + ")"; }
function offers_html(slot) {
    ...
    return list.map(function (o) { return "<span style='color:" + (o.same ? "#4caf50" : (o.wish > 0 ? "#8ab4f8" : "#e6e6e6")) + "' title='" + esc((o.wish > 0 ? "Wunschliste · " : "Alternative · ") + "Wert " + Math.round(o.score) + " · Händler " + o.seller + " auf " + o.server + (o.same ? " (dieser Server)" : "")) + "'>" + esc(o.name) + "+" + o.level + " " + fmt(o.price) + " <small>" + esc(o.server) + (o.same ? " ★" : "") + "</small></span>"; }).join("<br>");
}
...
function norm_server(sv) { return String(sv || "").replace(/\s+/g, "").toLowerCase(); }
...
async function scan_all_merchants(force, only_slot) {
    ...
    var res = null, merchants = [];
    try { res = await parent.api_call("pull_merchants", {}, { promise: true }); } catch (e) { res = null; }
    collect_merchants(res, merchants, 0);
    if (!merchants.length) { // api_call liefert nur {success:true}; die Daten kommen per direktem POST
        try {
            var r = await fetch("https://adventure.land/api/pull_merchants", { method: "POST", credentials: "include", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: "arguments=" + encodeURIComponent("{}") });
            var txt = await r.text(); try { res = JSON.parse(txt); } catch (e) { res = txt; }
            collect_merchants(res, merchants, 0);
        } catch (e) { game_log("Händler-Abfrage fehlgeschlagen: " + err_txt(e)); return; }
    }
    if (!global_debug_done) { global_debug_done = true; try { game_log("Händler-Abfrage: " + merchants.length + " Händler, Antwort-Typ " + (Array.isArray(res) ? "Array[" + res.length + "]" : typeof res) + (merchants[0] ? ", Felder: " + Object.keys(merchants[0]).slice(0, 12).join(",") : ", Rohdaten: " + JSON.stringify(res).slice(0, 160))); } catch (e) {} }
    ...
    finds.forEach(function (f) { var k = f.name + f.seller + f.price; if (!global_logged[k]) { global_logged[k] = true; game_log("Gefunden: " + f.name + "+" + f.level + " für " + fmt(f.price) + " bei " + f.seller + " (" + f.server + (f.same ? ", dieser Server – kaufe" : ", anderer Server") + ")"); } });

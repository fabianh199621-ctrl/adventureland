...
async function focus_slot(slot) { // Button "Jetzt": diesen Slot sofort verbessern – Wunschliste am Markt/Ponty, sonst getragenes Teil ausbauen
    ...
    try {
        ...
        if (WISHLIST[slot] && r0 < WISHLIST[slot].length) { await check_ponty(true); await check_market(true); }
...
async function scan_all_merchants() {
    if (Date.now() - last_global_scan < GLOBAL_SCAN_INTERVAL) return;

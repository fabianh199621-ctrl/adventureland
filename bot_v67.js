...
var MARKET_FACTOR = 1.5;             // Marktpreis ≈ Grundpreis x 1.5 (Annahme)
...
function update_panel() {
    ...
    if (panel.__gear) {
        ...
        h += "<table class='lp_t'><tr><th>Slot</th><th>Aktuell</th><th style='text-align:left'>Empfehlung (Gewinn/100k Gold)</th><th style='text-align:left'>Alternativen</th><th></th></tr>";
        for (var gs in goals) {
            ...
            h += "<tr><td>" + gs + "</td><td>" + (g.cur ? esc(g.cur.name) + "+" + (g.cur.level || 0) : "<span style='color:#ef5350'>leer</span>") + "</td>"
               + "<td style='text-align:left;white-space:normal;max-width:200px'>" + (rec ? esc(option_text(rec)) : "-") + "</td>"
...
async function check_ponty(force) {
    ...
    try {
        ...
        for (var i = 0; i < offer.length; i++) {
            ...
            var gl = goal_cache[slot] && goal_cache[slot].target, is_goal = gl && gl.item == o.name;
...
function compute_goals() {
    ...
    for (var slot in SLOT_TYPES) {
        ...
        if (cur && lvl < max_level(cur_def) && (cur_def.upgrade || cur_def.compound)) {
            ...
            if (gain > 0) options.push({ kind: "upgrade", item: cur.name, to: lvl + 1, gain: gain, cost: cost, ratio: gain / cost });
        }
        ...
        for (var i = 0; i < Math.min(cands.length, 80); i++) {
            ...
            if (farm_checked < 25) { var plan = plan_for(cd.name, 1, 0); if (plan) { farm_checked++; var hrs = plan.reduce(function (a, b) { return a + (b.hours || 0); }, 0); if (hrs <= GOAL_MAX_HOURS) { var c2 = hrs * gph + up_cost + (plan.some(function (st) { return st.type == "buy"; }) ? base_price(def2, cd.name) : 0); var o2 = { kind: "farm", item: cd.name, plan: plan, hours: hrs, gain: gain2, cost: c2, ratio: gain2 / Math.max(1, c2), pl: cd.pl }; if (!best_farm || o2.ratio > best_farm.ratio) best_farm = o2; } } }
            if (!is_buyable(cd.name) && base_price(def2, cd.name) <= budget) { var c3 = base_price(def2, cd.name) + up_cost; var o3 = { kind: "market", item: cd.name, price: base_price(def2, cd.name), gain: gain2, cost: c3, ratio: gain2 / Math.max(1, c3), pl: cd.pl }; if (!best_market || o3.ratio > best_market.ratio) best_market = o3; }
        }
        ...
        goal_cache[slot] = { cur: cur, options: options, target: best_farm ? { item: best_farm.item, plan: best_farm.plan, hours: best_farm.hours } : (best_market ? { item: best_market.item, plan: null, price: best_market.price, market: true } : null) };
    }
    return goal_cache;
}
function option_text(o) {
    var per = " (+" + Math.round(o.gain) + " für ~" + fmt(o.cost) + ", " + (o.ratio * 100000).toFixed(1) + "/100k)";
    if (o.kind == "upgrade") return "Upgrade auf +" + o.to + per;
    if (o.kind == "farm") return (o.hours > 0 ? "Farmen: " : "Beschaffen: ") + o.item + " ~" + (o.hours < 1 ? Math.round(o.hours * 60) + " min" : o.hours.toFixed(1) + " h") + per;
    if (o.kind == "market") return "Markt: " + o.item + " ~" + fmt(o.price) + per;
    if (o.kind == "buy") return "Kaufen: " + o.item + " " + fmt(o.price) + per;
    return o.kind;
}

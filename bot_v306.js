...
var merch_needs = {}, merch_need_q = {}, last_need_buy = 0; // Material, das der Händler angefragt hat (z. B. spidersilk für Werkzeuge) + Menge
...
function merch_need_tick() { // alle 2 min: Material, das der Händler braucht, auf dem Markt kaufen (er zahlt aus seiner Kasse, behält es selbst)
    ...
    for (var nm in merch_needs) {
        if (Date.now() - merch_needs[nm] > 24 * 3600000) { delete merch_needs[nm]; continue; }
        if (locate_item(nm) >= 0) continue; // habe ich selbst – geht bei der nächsten Abholung mit
        var o = merch_need_offer(nm); if (!o) continue;
...
function on_cm(name, data) { // Nachrichten der eigenen Charaktere
    ...
    try {
        ...
        else if (data.t == "bought") { if (merch_buy && data.id == merch_buy.id) { if (data.ok && merch_buy.for_merch) { delete merch_needs[merch_buy.o.name]; mb_done(merch_buy.o.name + " gekauft – bleibt beim Händler"); } else if (data.ok) { merch_buy.stage = "deliver"; merch_buy.t = Date.now(); mb_save(); } else mb_fail(data.why || "Kauf nicht gelungen"); } }
        ...
        else if (data.t == "need") { var np = get_player(name); var ni = locate_item(data.item); if (np && ni >= 0 && distance(character, np) < 400) { var nq = Math.min(data.q || 1, character.items[ni].q || 1); try { send_item(name, ni, nq); game_log("[" + who + "] " + nq + "x " + data.item + " übergeben"); } catch (e) {} } else if (ni < 0) { merch_needs[data.item] = Date.now(); merch_need_q[data.item] = data.q || 1; game_log("[" + who + "] braucht " + data.item + " – wird beim nächsten Fund/Abholung mitgegeben" + (merch_need_offer(data.item) ? ", oder er kauft es selbst auf dem Markt" : "")); } }
...
function merch_buy_tick() {
    ...
    if (j.stage == "away") { if (!arb_job) { if (j.for_merch) { delete merch_needs[j.o.name]; mb_done(j.o.name + " – Reise beendet, bleibt beim Händler"); return; } if (j.kind == "sell") { mb_done("Verkaufsreise beendet – Erlös liegt in der Händlerkasse"); return; } j.stage = "fetch"; j.t = Date.now(); j.tries = 0; mb_save(); } else if (el > 20 * 60000) mb_fail("Reise ohne Ende"); return; }
}

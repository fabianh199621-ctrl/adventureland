...
function update_panel() {
    ...
    h += "<div class='lp_row' style='flex-wrap:wrap;line-height:1.6'><span class='lp_k'>Wunschliste " + wish_text() + (AUTO_GEAR ? " · automatisch, Reserve " + fmt(WISH_RESERVE) : "") + ":</span> <span style='font-size:11px'>" + wish_html() + "</span></div>";
    if (panel.__gear) {
...
function compute_goals() {
    ...
    for (var slot in SLOT_TYPES) {
        ...
        var armor_cap = auto_mode && slot != "mainhand" && cur_def && cur_def.upgrade ? AUTO_ARMOR_MAX : 99;
...
var WISHLIST = {                      // je Slot: beste Wahl zuerst
...
function wish_status() { // je Slot: getragen / fehlt
    ...
    for (var slot in WISHLIST) {
        ...
        out.push({ slot: slot, have: r > 0, top: r == l.length, worn: name, want: r > 0 ? l[0] : l[0], inv: inv > r, seen: seen, rank: r, max: l.length });
    }
    return out;
}
...
async function focus_slot(slot) { // Button "Jetzt": diesen Slot sofort verbessern – Wunschliste am Markt/Ponty, sonst getragenes Teil ausbauen
    ...
    try {
        ...
        else if (worn && G.items[worn.name].upgrade) { check_pause(); await process_slot(slot, is_buyable(worn.name) ? UPGRADE_TARGET : (slot == "mainhand" ? WEAPON_SAFE_TARGET : SAFE_TARGET_DROP)); }
        worn = character.slots[slot];
...
async function scan_all_merchants(force, only_slot) {
    ...
    if (!merchants.length) { // api_call liefert nur {success:true}; die Daten kommen per direktem POST
        try {
            var r = await fetch("https://adventure.land/api/pull_merchants", { method: "POST", credentials: "include", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: "method=pull_merchants&arguments=" + encodeURIComponent("{}") });
    ...
    for (var pass = 0; pass < 60; pass++) {
        ...
        if (!best) {
            for (var i = 0; i < character.items.length; i++) {
                ...
                var cap2 = is_buyable(iv.name) ? UPGRADE_TARGET : (sl2 == "mainhand" ? WEAPON_SAFE_TARGET : SAFE_TARGET_DROP);
...
async function upgrade_routine_inner(manual) {
    ...
    try {
        ...
        if (mh && G.items[mh.name].upgrade) { check_pause(); await process_slot("mainhand", is_buyable(mh.name) ? UPGRADE_TARGET : WEAPON_SAFE_TARGET); }

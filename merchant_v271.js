...
async function do_pickup() { // zum Magier, Items entgegennehmen, Tränke/Gold übergeben, dann in der Stadt verarbeiten
    ...
    try {
        ...
        var gave_items = 0; for (var gi = character.items.length - 1; gi >= 0; gi--) { var git = character.items[gi]; if (git && (git.name == "anniversarygift" || /^slice_/.test(git.name))) { try { send_item(MAGE, gi, git.q || 1); gave_items += git.q || 1; await sleep(300); } catch (e) {} } }
        if (req.fetch && req.fetch.length) { for (var fi = 0; fi < req.fetch.length; fi++) { var fe = req.fetch[fi]; for (var fj = character.items.length - 1; fj >= 0; fj--) { var fit = character.items[fj]; if (fit && fit.name == fe.name && (fit.level || 0) == (fe.level || 0)) { try { send_item(MAGE, fj, fit.q || 1); gave_items += fit.q || 1; await sleep(350); } catch (e) {} } } delete hold_items[item_key(fe.name, fe.level)]; } hold_save(); if (gave_items) say("Einkauf übergeben: " + gave_items + " Item(s)"); else say("Einkauf: nichts zum Übergeben im Inventar"); }
...
async function process_inventory() { // in der Stadt: verkaufen, an den Stand, in die Bank
    ...
    try {
        ...
        if (bank2.length) { status("Bank"); stand_off(); for (var bt = 0; bt < 3 && character.map != "bank"; bt++) { try { await smart_move("bank"); } catch (e) { say("Weg zur Bank (" + (bt + 1) + "): " + (e && (e.reason || e.message) || JSON.stringify(e)) + " – Karte " + character.map); await sleep(1500); } } for (var w2 = 0; w2 < 25 && !character.bank; w2++) await sleep(200); var nb = 0, nf = 0, last_err = character.bank ? "" : "Bankdaten nicht geladen (Karte " + character.map + ")"; if (character.bank) for (var b2 = bank2.length - 1; b2 >= 0; b2--) { var e0 = character.esize; try { var pr = bank_store(bank2[b2]); if (pr && pr.then) pr.then(null, function (e) { last_err = String(e && e.reason || e); }); } catch (e) { last_err = String(e && e.reason || e); } await sleep(600); if (character.esize > e0) nb++; else nf++; } say("In die Bank gelegt: " + nb + " Items" + (nf ? ", " + nf + " nicht abgelegt (" + (last_err || "kein Fehler gemeldet") + ")" : "")); }

...
function item_tip(it) {
    ...
    if (def.g) lines.push("Wert " + fmt(def.g));
...
async function check_market(force) {
    ...
    try {
        ...
        for (var i = 0; i < offers.length; i++) {
            ...
            await sleep(1500);
            if (character.esize < before) { bought_slots[slot] = true; var idx = find_inv_index(o.name, o.level); if (idx >= 0) { equip(idx, slot); await sleep(600); bought++; } }
        }

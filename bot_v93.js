...
async function travel_place(name) { // Orte in der Stadt: bei großer Entfernung teleportieren
    try {
        ...
        if (ts && skill_available("town") && !is_on_cooldown("town") && !character.rip && (character.map != "main" || distance(character, ts) > 900)) {
            set_message("Teleport"); use_skill("town"); var t0 = Date.now(); await sleep(4500);
            while (is_moving(character) && Date.now() - t0 < 8000) await sleep(300);
        }
    } catch (e) {}
...
async function travel(dest) { // dest: Monstername, Ortsname oder {map,x,y}
    ...
    try {
        if (pos && pos.map && !character.rip && skill_available("town") && !is_on_cooldown("town")) {
            ...
            if (ts && here_to_town > 600 && via_town + 600 < direct) {
                set_message("Teleport"); use_skill("town");
                var t0 = Date.now(); await sleep(4500);
                while (is_moving(character) && Date.now() - t0 < 8000) await sleep(300);
            }
        }
    } catch (e) {}

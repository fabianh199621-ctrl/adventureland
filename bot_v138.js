...
function init_panel() {
    ...
    var onClick = function (e) {
        ...
        else if (act == "focus") { focus_mode = !focus_mode; try { localStorage.setItem("lp_focus", focus_mode ? "1" : "0"); } catch (x) {} tidy_next = 0; game_log("Fokus-Modus " + (focus_mode ? "an – nur farmen/hunten, Nebenroutinen aus" : "aus – alle Routinen wieder aktiv")); }
...
async function tidy_inventory() {
    ...
    try {
        ...
        if (junk.length || dups.length || focus_mode) {
            ...
            for (var j = 0; j < junk.length; j++) { var it = character.items[junk[j]]; if (!it) continue; var g0 = character.gold, snap = { name: it.name, level: it.level, q: it.q }; sell(junk[j], it.q || 1); await sleep(300); try { note_sell(snap, character.gold - g0); } catch (e) {} }

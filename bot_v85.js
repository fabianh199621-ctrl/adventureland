...
game_log("LogicPlan-Skript " + BOT_VERSION + " gestartet – PAUSIERT. P = Start/Pause, N = neu laden, U = Upgrades nach Empfehlung, L = Statistik, G = Gifts tauschen");
...
function item_tip(it) {
    ...
    if (it.stat_type) lines.push("Attribut: " + it.stat_type);
...
function update_panel() {
    ...
    h += "<div class='lp_row'><span class='lp_mode' style='color:#9aa3b2'>Hunt: <span style='color:#e6e6e6'>" + esc(mh_txt()) + "</span> · " + tokens() + " Tokens</span><button data-act='hunt'" + (hunt_on ? " class='on'" : "") + " title='Monster Hunt automatisch'>Hunt</button>" + (mh() ? "<button data-act='huntabandon'>Abbrechen</button>" : "") + "</div>";
...
async function exchange_gifts() {
    ...
    try {
        ...
        if (!todo.length) { game_log("Nichts zum Tauschen"); }
            ...
            set_message("Zu Xyn"); stop("smart"); await travel_place("exchange");
            var done = 0, fails = 0;
            while (!paused) {
                var idx = -1;
...
// ---------- Muscheln bei Xyn gegen Elixiere tauschen ----------
var shelling = false;
async function check_seashells() {
    if (shelling || busy || upgrading || paused || quantity("seashell") < SHELLS_PER_TRIP) return;
    shelling = true; busy = true;
    try {
        game_log("Tausche " + quantity("seashell") + " Muscheln bei Xyn");
        set_message("Muscheln"); stop("smart"); await travel_place("exchange");
        var done = 0, fails = 0;
    ...
    } catch (e) { game_log("Muschel-Tausch-Fehler: " + e); }
...
function mh() { return character.s && character.s.monsterhunt; }
function mh_txt() { var q = mh(); if (!q) return "keine Jagd"; return q.id + " " + (q.c || 0) + " übrig · " + fmt_time(q.ms || 0); }
...
async function check_monsterhunt() {
    ...
    try {
        ...
        if (!q2) { try { r = await interact("monsterhunt"); } catch (e) { r = e; } await sleep(800); q2 = mh(); }
        if (q2 && q2.c > 0) {
            if (hunt_target_ok(q2.id)) { hunt_bad_tries = 0; game_log("Neue Jagd: " + q2.c + "x " + q2.id + " (" + fmt_time(q2.ms || 0) + ")"); }
                ...
                if (mh() && mh().c > 0) { game_log("Aufgeben nicht möglich – Jagd läuft aus, normal weiterfarmen"); hunt_cooldown_until = Date.now() + (mh().ms || 1800000); }
        ...
        await spend_tokens();
        if (hunt_spot && !(mh() && mh().c > 0)) { manual_spot = hunt_prev ? hunt_prev.manual : null; hunt_prev = null; hunt_spot = null; need_repick = true; current_spot = null; }
    } catch (e) { game_log("Monster-Hunt-Fehler: " + e); hunt_cooldown_until = Date.now() + 5 * 60000; }
    hunting = false; busy = false;
    if (!paused) go_to_farm_spot();
}
async function hunt_abandon() {
    if (!mh()) { game_log("Keine Jagd aktiv"); return; }
    busy = true; try { await daisy(); await interact("monsterhunt"); await sleep(800); game_log(mh() ? "Aufgeben nicht möglich" : "Jagd aufgegeben"); } catch (e) { game_log("Fehler: " + e); }
...
async function run_goal() {
    ...
    try {
        if (st.type == "buy") { await buy_items(st.item, st.qty); }
            ...
            set_message("Tauschen"); await travel_place("exchange");
            var tries = 0;

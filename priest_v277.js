...
async function go_give_mage() { // alles außer Tränken/Tokens/Tracker zum Magier bringen und übergeben
    ...
    try {
        ...
        for (var j = character.items.length - 1; j >= 0; j--) { var it = character.items[j]; if (!it) continue; if (/^(hpot|mpot)/.test(it.name) || it.name == "tracker" || it.name == "computer") continue;
            if (it.name == "monstertoken" && have_item("tracker") < 0) { say("Tokens behalte ich (" + (it.q || 1) + "/4) – kaufe mir damit bei Daisy einen Tracktrix für Cavalry"); continue; }
...
async function hunt_town_step() { // in der Stadt: Jagd abgeben/holen, Tokens in Set-Teile für den Magier umsetzen (in die Bank)
    ...
    q = mh_q();
    if (!q) { if (hunt_may_take()) { try { await interact("monsterhunt"); } catch (e) {} await sleep(800); q = mh_q(); if (q) say("Neue Jagd: " + q.c + "x " + q.id); } else say("Keine neue Jagd geholt: " + (mage && mage.hunt_holder ? mage.hunt_holder + " läuft" : "ein anderer holt")); }
    // Tokens: erst eigenen Tracktrix (4 Tokens, für Cavalry), dann fehlende Set-Teile des Magiers
    if (have_item("tracker") < 0 && pot_count("monstertoken") >= 4 && character.esize >= 1) { try { exchange_buy("monstertoken", "tracker"); await sleep(1500); } catch (e) {} if (have_item("tracker") >= 0) say("Tracktrix für 4 Tokens gekauft – Cavalry verfügbar"); }

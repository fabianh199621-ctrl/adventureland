...
try { var rs = parent.__lp_resume; if (rs && Date.now() - rs.t < 60000) { paused = !!rs.paused; } parent.__lp_resume = null; } catch (e) {}
...
function save_state() {
    try { localStorage.setItem("lp_state_" + character.name, JSON.stringify({ t: Date.now(), spot: current_spot, meas: meas, manual: manual_spot, hunt_prev: hunt_prev, hunt_spot: hunt_spot })); } catch (e) {}
}
...
function reload_bot() {
    ...
    fetch(BOT_BASE + "version.txt?t=" + Date.now(), { cache: "no-store" })
        ...
        .then(function (code) {
            parent.__lp_resume = { paused: paused, t: Date.now() }; // Zustand für die neue Version
...
function init_panel() {
    ...
    var onClick = function (e) {
        ...
        else if (act == "huntabandon") hunt_abandon();
        ...
        else if (act == "bank") bank_cleanup_now();
        else if (act == "banksort") bank_sort_now();
...
function is_junk(it) { // kaufbare Standardausrüstung ohne Level/Attribut; ungetragener kaufbarer Schmuck +0 in Einzelstücken; fremde Elixiere
...
function dup_protected(it) { var def = G.items[it.name] || {}; return it.p || it.stat_type || (it.level || 0) > DUP_MAX_LEVEL || !def.upgrade || EVENT_ITEMS.test(it.name); }
...
function surplus_copies(name) { return all_copies(name).filter(function (c) { return !dup_protected(c.it); }).slice(dup_keep(name)); }
...
async function tidy_inventory() {
    ...
    try {
        ...
        if (character.esize < INV_TARGET_FREE && has_compound_triples()) { try { await compound_spares(); } catch (e) { if (e == "PAUSE") throw e; } }
        // 3. Rest in die Bank (vorher: Teile für leere Slots zurückholen) – so lange, bis INV_TARGET_FREE Plätze frei sind
        if (character.esize < INV_TARGET_FREE) {
            ...
            if (character.esize < INV_TARGET_FREE) {
                var order = [];
                for (var k2 = 0; k2 < character.items.length; k2++) { var it3 = character.items[k2]; if (it3 && bankable_extra(it3)) order.push({ i: k2, r: extra_rank(it3) }); }
                order.sort(function (a, b) { return a.r - b.r; });
                for (var o = 0; o < order.length && character.esize < INV_TARGET_FREE; o++) { bank_store(order[o].i); extra++; await sleep(300); }
            }
            game_log("Inventar: " + (n + extra) + " Items in die Bank gelegt" + (extra ? " (davon " + extra + " Schmuck/Reserven)" : "") + " – frei: " + character.esize);
...
function has_compound_triples() {
    var eq = equipped_names(), g = {};
    for (var i = 0; i < character.items.length; i++) { var it = character.items[i]; if (!it) continue; var def = G.items[it.name]; if (!def || !def.compound || eq[it.name] || (it.level || 0) >= COMPOUND_SPARE_MAX) continue; var k = it.name + "/" + (it.level || 0); g[k] = (g[k] || 0) + 1; if (g[k] >= 3) return true; }
    return false;
}
function bankable_extra(it) {
    var def = G.items[it.name]; if (!def || KEEP_ITEMS.test(it.name) || EVENT_ITEMS.test(it.name)) return false;
    var eq = equipped_names();
    if (def.compound && !eq[it.name]) return true;                      // ungetragener Schmuck (wartet auf Dreiergruppe)
    if (eq[it.name] && def.upgrade) return backup_index(it.name) != character.items.indexOf(it); // Reserven: nur die beste bleibt
    return false;
}
...
function err_txt(e) { try { return typeof e == "string" ? e : (e && (e.reason || e.message)) ? (e.reason || e.message) : JSON.stringify(e).slice(0, 120); } catch (x) { return String(e); } }
...
var shelling = false, shells_retry_at = 0;
...
// ---------- Monster Hunt (Daisy) ----------
var hunting = false, last_hunt_check = 0, hunt_cooldown_until = 0, hunt_prev = null, hunt_spot = null, hunt_bad_tries = 0;
...
async function check_monsterhunt() {
    ...
    if (q && q.c > 0) {
        if (!hunt_target_ok(q.id)) { if (hunt_spot) { manual_spot = hunt_prev ? hunt_prev.manual : null; hunt_spot = null; need_repick = true; } return; } // unsicher -> läuft ab, normal weiterfarmen
        if (q.ms && q.ms < MH_MIN_LEFT_MS && q.c > 3) return; // kaum noch Zeit: nicht mehr wechseln
        if (hunt_spot != q.id) { hunt_prev = hunt_prev || { manual: manual_spot }; hunt_spot = q.id; game_log("Monster Hunt: " + q.c + "x " + q.id + " (" + fmt_time(q.ms || 0) + ")"); set_manual_spot(q.id); }
        return;
    }
    ...
    try {
        ...
        await spend_tokens();
        if (hunt_spot && !(mh_quest() && mh_quest().c > 0)) { manual_spot = hunt_prev ? hunt_prev.manual : null; hunt_prev = null; hunt_spot = null; need_repick = true; current_spot = null; }
...
async function hunt_abandon() {
    if (!mh_quest()) { game_log("Keine Jagd aktiv"); return; }
    busy = true; try { await daisy(); await interact("monsterhunt"); await sleep(800); game_log(mh_quest() ? "Aufgeben nicht möglich" : "Jagd aufgegeben"); } catch (e) { game_log("Fehler: " + e); }
    busy = false; hunt_cooldown_until = Date.now() + 60000;
    if (hunt_spot) { manual_spot = hunt_prev ? hunt_prev.manual : null; hunt_prev = null; hunt_spot = null; need_repick = true; current_spot = null; }
    if (!paused) go_to_farm_spot();
}
    ...
    for (var t = 0; t < todo.length; t++) {
        ...
        for (var l = 0; l < COMPOUND_SPARE_MAX; l++) {
            while (spendable() > 0) {
                ...
                if (sc < 0) { game_log("keine " + scroll); break; }
                set_message(name + " +" + l + " x3");
                var prev_n1s = find_inv_indices(name, l + 1).length;
                try { await compound(idx[0], idx[1], idx[2], sc); } catch (e) {}
                await wait_queue("compound");
                if (find_inv_indices(name, l + 1).length > prev_n1s) { record_attempt("c", l, true); game_log(name + " +" + (l + 1) + " erstellt"); }
                else { record_attempt("c", l, false); game_log(name + " compound (+" + l + ") fehlgeschlagen"); }
            }
        }
    }
}
...
async function upgrade_routine() {
    var manual = false;
    if (upgrading) { game_log("Upgrade läuft bereits"); return; }
    if (busy) { pending_upgrade = "U"; unpause("Upgrade vorgemerkt"); game_log("Upgrade vorgemerkt – startet, sobald er frei ist"); return; }
    ...
    game_log("Upgrade-Routine: " + empty + " leere Slots, " + up_slots.length + " Upgrades, " + stat_slots.length + " Attribut, " + comp_slots.length + " Compound (frei: " + spendable() + " Gold)");
...
function start_main() {
  ...
  main_timer = parent.__lp_main_timer = setInterval(function () {
    ...
    if (pending_upgrade && !busy && !upgrading) { pending_upgrade = null; upgrade_routine(); }
    run_goal(); check_weapon(); check_gear_slots(); check_flee(); check_elixir(); kiss_routine(); tidy_inventory(); check_potions(); check_stuck(); check_ponty(false); check_market(false); check_seashells(); check_monsterhunt(); check_cake();

    ...
    if (panel.__gear) {
        ...
        for (var gs in goals) {
            ...
            h += "<tr><td>" + gs + "</td><td>" + (g.cur ? esc(g.cur.name) + "+" + (g.cur.level || 0) : "<span style='color:#ef5350'>leer</span>") + "</td>"
               + "<td>" + (t ? esc(t.item) : "-") + "</td><td style='text-align:left;white-space:normal;max-width:190px'>" + (t ? esc(plan_text(t.plan, t)) + (t.plan && t.hours ? " – gesamt ~" + (t.hours < 1 ? Math.round(t.hours * 60) + " min" : t.hours.toFixed(1) + " h") : "") : "-") + "</td>"
...
function compute_goals() { // je Slot: aktuelles Item, nächstbesseres machbares Ziel, Plan
    ...
    for (var slot in SLOT_TYPES) {
        ...
        if (!pick && !fallback) { // realistisches Marktziel: bestes Item mit Grundpreis <= halbes freies Gold
            var budget = Math.max(20000, spendable() * 0.5);
            for (var j = 0; j < cands.length; j++) { var gd = G.items[cands[j].name]; if ((gd.g || 0) <= budget && !(cur && cands[j].name == cur.name)) { pick = { item: cands[j].name, score: cands[j].score, plan: null, market: true, price: gd.g }; break; } }
        }
        if (!pick) pick = fallback || null;

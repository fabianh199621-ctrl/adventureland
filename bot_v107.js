...
function reload_bot() {
    ...
    fetch(BOT_BASE + "version.txt?t=" + Date.now(), { cache: "no-store" })
        ...
        .then(function (code) {
            parent.__lp_resume = { paused: paused, t: Date.now(), timers: { last_ponty: last_ponty, last_market: last_market, shells_retry_at: shells_retry_at, hunt_cooldown_until: hunt_cooldown_until, cake_next: cake_next, last_hunt_check: last_hunt_check } }; // Zustand für die neue Version
...
function busy_watchdog() { // hängt eine Routine (busy ohne Ende), nach 4 min freigeben
    ...
    else if (Date.now() - busy_since > 4 * 60000) { busy_since = 0; game_log("Routine hängt – gebe frei"); busy = false; upgrading = false; kissing = false; exchanging = false; pontying = false; marketing = false; shelling = false; hunting = false; bank_cleaning = false; sorting_inv = false; baking = false; }
}
...
var WISHLIST = {                      // je Slot: beste Wahl zuerst
    mainhand: ["harbringer", "firestaff", "froststaff", "mushroomstaff"],
...
async function wish_buy_from_offer(o, buy_fn) { // o: {name, level, price, ...}; buy_fn: async -> true bei Erfolg
    ...
    if (idx >= 0) { try { equip(idx, slot); await sleep(800); game_log("Wunschliste: " + o.name + "+" + bl + " angelegt (" + slot + ") – jetzt " + wish_text()); } catch (e) {} }
    return true;
}
...
function stat_weights(st, stat_type) { // stat_type: Attributrichtung des konkreten Items (Scroll); "stat" zählt nur, wenn sie zum Hauptattribut passt
    ...
    return (main + (stat_ok ? (st.stat || 0) : 0)) * 30 + (st.attack || 0) * 3 + (st.range || 0) * 0.5 + (st.frequency || 0) * 5 + (st.hp || 0) * 0.05 + (st.mp || 0) * 0.1 + (st.armor || 0) * 0.5 + (st.resistance || 0) * 0.5 + (st.rpiercing || 0) * 2;
}
...
async function ensure_backup(name) { // Reservekopie auf +BACKUP_LEVEL herstellen
    for (var tries = 0; tries < 3; tries++) {
        ...
        if (base < 0) { if (!await buy_items(name, 1)) return false; await travel_place("upgrade"); }
    ...
    for (var pass = 0; pass < 60; pass++) {
        ...
        if (!best) {
            for (var i = 0; i < character.items.length; i++) {
                ...
                var lv2 = iv.level || 0; if (lv2 >= UPGRADE_TARGET) continue;
                if (gear_score(G.items[iv.name], UPGRADE_TARGET) <= item_score(worn) * 1.05) continue;
        ...
        if (now && now.name == name && (now.level || 0) <= lvl && !buyable) skipped[best.slot] = true; // Drop-Item: nach Fehlschlag nicht endlos weiter
    }
}
async function upgrade_routine(auto_run) {
    ...
    try { await upgrade_routine_inner(manual); } finally { GOLD_RESERVE = saved_reserve; }
}
...
function start_main() {
  ...
  main_timer = parent.__lp_main_timer = setInterval(function () {
    ...
    busy_watchdog(); auto_gear_tick(); run_goal(); check_weapon(); check_gear_slots(); check_flee(); check_elixir(); kiss_routine(); tidy_inventory(); check_potions(); check_stuck(); check_ponty(false); check_market(false); check_seashells(); check_monsterhunt(); check_cake();

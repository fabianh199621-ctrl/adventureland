...
async function upgrade_by_recommendation() {
    ...
    for (var pass = 0; pass < 40; pass++) {
        check_pause();
        goal_cache_t = 0; var goals = compute_goals(), any = false;
        for (var slot in goals) {
            var rec = goals[slot].options && goals[slot].options[0]; if (!rec || rec.kind != "upgrade" || skipped[slot]) continue;
            var it = character.slots[slot]; if (!it) continue;
            var name = it.name, lvl = it.level || 0, buyable = is_buyable(name), def = G.items[name];
            if (def.compound) continue; // Schmuck macht compound_slot
            if (!buyable && lvl >= SAFE_TARGET_DROP && backup_index(name) < 0) { game_log(name + ": Drop-Item ohne Reserve – bleibt bei +" + lvl); skipped[slot] = true; continue; }
            if (rec.cost > spendable()) { game_log(name + " +" + (lvl + 1) + ": zu teuer (~" + fmt(rec.cost) + ")"); skipped[slot] = true; continue; }
            await smart_move("upgrade");
            await process_slot(slot, lvl + 1);
            any = true;
        }
        if (!any) break;
    }

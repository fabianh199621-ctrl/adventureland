...
async function process_inventory() { // in der Stadt: verkaufen, an den Stand, in die Bank
    ...
    try {
        ...
        if (bank2.length) { status("Bank"); stand_off(); await smart_move("bank"); for (var w2 = 0; w2 < 25 && !character.bank; w2++) await sleep(200); var nb = 0, nf = 0, last_err = character.bank ? "" : "Bankdaten nicht geladen (Karte " + character.map + ")"; if (character.bank) for (var b2 = bank2.length - 1; b2 >= 0; b2--) { var e0 = character.esize; try { var pr = bank_store(bank2[b2]); if (pr && pr.then) pr.then(null, function (e) { last_err = String(e && e.reason || e); }); } catch (e) { last_err = String(e && e.reason || e); } await sleep(600); if (character.esize > e0) nb++; else nf++; } say("In die Bank gelegt: " + nb + " Items" + (nf ? ", " + nf + " nicht abgelegt (" + (last_err || "kein Fehler gemeldet") + ")" : "")); }

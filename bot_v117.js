...
function init_panel() {
    ...
    var onClick = function (e) {
        ...
        if (wl) { var wt = wl.getAttribute("data-wtab"), wv = wl.getAttribute("data-wiki"); if (wt) { wiki.tab = wt; wiki.page = null; wiki.hist = []; render_wiki(); } else if (wv == "back") wiki_back(); else { var ci = wv.indexOf(":"); wiki_go(wv.slice(0, ci), wv.slice(ci + 1)); } return; }
...
function compute_goals() {
    ...
    for (var slot in SLOT_TYPES) {
        ...
        var armor_cap = cur && wish_item(slot) == cur.name ? wish_level(slot) : (auto_mode && slot != "mainhand" && cur_def && cur_def.upgrade ? AUTO_ARMOR_MAX : 99);
...
var wiki_panel = null, wiki = { q: "", tab: "items", page: null, hist: [] };
...
async function upgrade_routine_inner(manual) {
    ...
    var weapon_todo = character.slots.mainhand && G.items[character.slots.mainhand.name].upgrade && (character.slots.mainhand.level || 0) < (is_buyable(character.slots.mainhand.name) ? UPGRADE_TARGET : WEAPON_SAFE_TARGET);
    ...
    try {
        ...
        if (mh && G.items[mh.name].upgrade) { check_pause(); await process_slot("mainhand", slot_target("mainhand", is_buyable(mh.name) ? UPGRADE_TARGET : WEAPON_SAFE_TARGET, 0)); }

...
async function go_tidy() { // Aufräumen auf Befehl des Magiers: Schrott verkaufen, Rest in die Bank, Tränke und Tokens behalten
    ...
    try {
        ...
        var stored = 0;
        var failed = 0;
        if (bank.length) { try { await smart_move("bank"); await sleep(800); for (var b = bank.length - 1; b >= 0; b--) { var ib = character.items[bank[b]]; if (!ib) continue; var e0 = character.esize; try { bank_store(bank[b]); } catch (e) {} await sleep(400); if (character.esize > e0) stored++; else failed++; } } catch (e) { say("Bank: " + (e && e.message || e)); } }
        say("Aufgeräumt: " + sold + " verkauft, " + stored + " in die Bank" + (failed ? ", " + failed + " nicht abgelegt (Bank voll?)" : "") + " – frei " + character.esize);

...
function build_vs_buy(slot) {
    ...
    else if (over) verdict = (build.possible ? "bauen" : "warten") + " – Angebot über Preisgrenze (" + fmt(best.price) + ")";
...
function wish_wants(name, level, price) { // Slot, für den dieses Angebot die Wunschliste verbessert – sonst null
    ...
    for (var slot in WISHLIST) {
        ...
        var cap = Math.max(WISH_MAX[name] || 0, def.g * 8, is_buyable(name) ? 0 : own); // nicht kaufbare: bis zu den eigenen Baukosten bis zur angebotenen Stufe (inkl. Risiko)

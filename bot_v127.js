...
function update_panel() {
    ...
    try { var wantW = (panel.__wish || panel.__gear) ? 820 : 470; if (panel.__w != wantW) { panel.__w = wantW; panel.style.width = wantW + "px"; clamp_pos(panel, panel.offsetLeft, panel.offsetTop); } } catch (e) {}
...
function build_vs_buy(slot) {
    ...
    else if (!best) verdict = build.possible ? "bauen (kein Angebot)" : (/Tokens/.test(build.note) ? "Tokens sammeln (Monster Hunt)" : "warten (kein Angebot, keine Basis)");
...
function wish_ui_html() { // Zielbau-Tabelle
    var h = "<table class='lp_t' style='table-layout:fixed;width:100%'><tr><th style='text-align:left;width:60px'>Slot</th><th style='text-align:left;width:160px'>Zielitem (Tooltip = Werte)</th><th>Stufe</th><th style='text-align:left'>Stand</th><th style='text-align:left'>Bauen oder kaufen?</th><th style='text-align:left'>Angebote alle Server (★ = hier)</th><th></th></tr>";
    for (var slot in SLOT_TYPES) {

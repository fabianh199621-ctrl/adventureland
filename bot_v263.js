...
function init_panel() {
    ...
    var onChange = function (e) {
        ...
        if (t.getAttribute("data-mgk")) { var mgk2 = t.getAttribute("data-mgk"); if (mgk2 == "esc") { var ev = parseInt(String(t.value).replace(/\D/g, "")) || 0; if (ev >= 10) MG.esc_max = ev * 1000; } else { var mv3 = parse_mio(t.value); if (mv3 > 0) MG[mgk2] = mv3; } if (MG.min > MG.target) MG.min = MG.target; mg_save(); game_log("Händler-Kasse: Ziel " + fmt(MG.target) + ", nachfüllen unter " + fmt(MG.min) + ", Priest/Ranger max. " + fmt(MG.esc_max)); try { t.blur(); } catch (x) {} return; }
...
function update_panel() {
    ...
    h += lp_sec("haendler", "Händler", stand_txt + (sc ? " · " + sc : "") + " · " + (tips.length ? tips.length + " Serverfund" + (tips.length > 1 ? "e" : "") : "kein Serverfund") + (arb_auto ? " · Handelsreise Auto" : ""),
        (merch_buy ? "<div class='lp_row'><span class='lp_k'>Einkauf:</span> <span style='color:#8ab4f8'>" + esc(mb_stage_txt()) + "</span><button data-act='mbcancel' title='Einkauf abbrechen (Gold kommt bei der nächsten Abholung zurück)'>Abbrechen</button></div>" : "") + "<div class='lp_row'><span class='lp_k'>Aktionen</span>" + (team_on.merch ? "<button data-act='goldback' title='Händler bringt alles über seinem Zielbestand, Priest/Ranger alles über ihrem Maximum zu dir'>Gold holen" + (mst0 && mst0.gold ? " (" + fmt(mst0.gold) + ")" : "") + "</button>" : "") + "<span class='lp_k' style='margin-left:6px'>Kasse:</span> <input data-mgk='target' value='" + esc(fmt_mio(MG.target)) + "' title='Zielbestand des Händlers in Mio. – er füllt bei dir auf, wenn er unter die Nachfüllgrenze fällt' style='width:44px;font-size:11px;background:#1c2029;color:#eee;border:1px solid #555'> <span class='lp_k'>Mio · nachfüllen unter</span> <input data-mgk='min' value='" + esc(fmt_mio(MG.min)) + "' title='Nachfüllgrenze des Händlers in Mio.' style='width:44px;font-size:11px;background:#1c2029;color:#eee;border:1px solid #555'> <span class='lp_k'>Mio · Priest/Ranger max.</span> <input data-mgk='esc' value='" + esc(String(Math.round(MG.esc_max / 1000))) + "' title='Priest/Ranger geben alles über diesem Betrag (in Tausend) an dich ab' style='width:44px;font-size:11px;background:#1c2029;color:#eee;border:1px solid #555'> <span class='lp_k'>k</span>" + "<button data-act='mkttoggle' title='Markt-Fenster: alle Angebote aller Server, filterbar'>Markt</button><button data-act='marketscan' title='Alle Händlerstände jetzt abfragen: Zielbau-Angebote und Schnäppchen unter NPC-Wert'" + (scanning_now ? " class='on'" : "") + ">" + (scanning_now ? "Scan läuft…" : "Schnäppchen scannen") + "</button></div>"
...
function mkt_compact_html() {
    ...
    groups.slice(0, lim).forEach(function (gr) {
        ...
        var act = a ? mkt_action_html(a, idx(a)) : "";
...
async function run_market_job() { // Klick im Markt-Fenster: hinlaufen und kaufen bzw. nur hinlaufen
    ...
    try {
        set_message(j.mode == "goto" ? "Markt: hin" : "Markt: Kauf");
        if (o.map && o.x != null) await travel({ map: o.map, x: o.x, y: o.y + 30 });
        if (j.mode == "goto") { paused = true; set_message("PAUSE"); game_log("Markt: bei " + o.seller + " – Bot pausiert, damit du in Ruhe schauen kannst (Pause aufheben zum Weiterfarmen)"); }
        else {
...
var MG = { target: 5000000, min: 1000000, esc_max: 100000 }; try { var mg0 = JSON.parse(localStorage.getItem("lp_merch_gold") || "null"); if (mg0) for (var mgk in mg0) MG[mgk] = mg0[mgk]; } catch (e) {} // Händler-Kasse: Ziel, Nachfüllgrenze, Obergrenze Priest/Ranger

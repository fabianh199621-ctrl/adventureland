...
function success_p(kind, level) { var st = up_stats[kind + level], def = kind == "u" ? (UP_P[level] != null ? UP_P[level] : 0.15) : (CO_P[level] != null ? CO_P[level] : 0.25); if (st && st.n >= 5) return (st.ok + 1) / (st.n + 2); return def; }
function success_txt(kind, level) { var st = up_stats[kind + level]; return Math.round(success_p(kind, level) * 100) + " %" + (st && st.n ? " (" + st.ok + "/" + st.n + ")" : " (Tabelle)"); }
...
function event_debug() {
    ...
    try { out.merchant_keys = Object.keys(parent).filter(function (k) { return /merch|api|pull/i.test(k); }).map(function (k) { return k + ":" + typeof parent[k]; }); out.api_call_src = String(parent.api_call || "").slice(0, 600); } catch (e) {}
...
function reload_bot() {
    ...
    fetch(BOT_BASE + "version.txt?t=" + Date.now(), { cache: "no-store" })
        ...
        .then(function (code) {
            parent.__lp_resume = { paused: paused, t: Date.now(), timers: { last_ponty: last_ponty, last_market: last_market, shells_retry_at: shells_retry_at, hunt_cooldown_until: hunt_cooldown_until, cake_next: cake_next, last_hunt_check: last_hunt_check, last_global_scan: last_global_scan } }; // Zustand für die neue Version
...
function toggle_pause() {
    paused = !paused;
    if (paused) {
        stop("move"); stop("smart"); if (!routine_running()) busy = false; // laufende Routinen beenden sich selbst (check_pause)
        set_message("PAUSE");
...
async function travel(dest) { // dest: Monstername, Ortsname oder {map,x,y}
    ...
    catch (e) { await sleep(2000); stop("smart"); return smart_move(dest); } // einmal erneut versuchen
}
...
function init_panel() {
    ...
    try { div.__wish = localStorage.getItem("lp_panel_wish") == "1"; } catch (e) { div.__wish = false; }
    ...
    var onChange = function (e) {
        if (!inside(e)) return;
        var t = e.target; if (!t || t.tagName != "SELECT") return;
        var ws = t.getAttribute("data-wslot"), wl = t.getAttribute("data-wlvl");
        if (ws) { var v = t.value; if (!v) wish_cfg[ws] = { item: "" }; else wish_cfg[ws] = { item: v, level: default_target_level(G.items[v]) }; save_wish_cfg(); game_log("Zielbau " + ws + ": " + (v ? v + " +" + wish_cfg[ws].level : "kein Ziel")); }
    ...
    var onKeyCap = function (e) { var t = e.target; if (t && t.id == "lp_wiki_q") { e.stopPropagation(); if (e.key == "Escape") t.blur(); } }; // Tasten im Suchfeld nicht ans Spiel/Bot weitergeben
...
function update_panel() {
    ...
    var st_txt = !bot_running ? "AUS" : paused ? "PAUSE" : upgrading ? "Upgrade" : pending_upgrade ? "Upgrade wartet" : kissing ? "Kuss" : fleeing ? "Rückzug" : exchanging ? "Tausch" : pontying ? "Ponty" : marketing ? "Markt" : hunting ? "Daisy" : busy ? "unterwegs" : "farmt";
    ...
    h += "<div class='lp_row'><span class='lp_mode' style='color:#9aa3b2'>Hunt: <span style='color:#e6e6e6'>" + esc(mh_txt()) + "</span> · " + tokens() + " Tokens</span><button data-act='hunt'" + (hunt_on ? " class='on'" : "") + " title='Monster Hunt automatisch'>Hunt</button>" + (mh_quest() ? "<button data-act='huntabandon'>Abbrechen</button>" : "") + "</div>";
    ...
    h += "<div class='lp_row'><span class='lp_mode' style='color:#9aa3b2'>Zielbau " + wish_text() + (AUTO_GEAR ? " · automatisch, Reserve " + fmt(WISH_RESERVE) : "") + "</span><button data-act='wishtoggle'>" + (panel.__wish ? "▾" : "▸") + " Zielbau</button></div>";
    if (panel.__wish) h += wish_ui_html();
    if (panel.__gear) {
        var goals = compute_goals();
        h += "<table class='lp_t'><tr><th>Slot</th><th>Aktuell</th><th style='text-align:left'>Empfehlung (Wert je Aufwand)</th><th style='text-align:left'>Alternativen</th><th style='text-align:left'>Angebote alle Server (★ = hier)</th><th></th></tr>";
        for (var gs in goals) {
            ...
            h += "<tr><td>" + gs + "</td><td>" + (g.cur ? esc(g.cur.name) + "+" + (g.cur.level || 0) : "<span style='color:#ef5350'>leer</span>") + "</td>"
               ...
               + "<td style='text-align:left;white-space:normal;max-width:190px'>" + offers_html(gs) + "</td>"
               + "<td><button data-act='focus' data-slot='" + gs + "' title='diesen Slot jetzt verbessern: Wunschliste kaufen, sonst Compound/Upgrade'>Jetzt</button> " + (t && t.plan ? "<button data-act='goal' data-slot='" + gs + "'" + (gear_goal && gear_goal.slot == gs ? " class='on'" : "") + ">Beschaffen</button>" : "") + (t ? " <button data-act='goalskip' data-item='" + t.item + "' title='dieses Item überspringen' style='padding:1px 5px'>✕</button>" : "") + "</td></tr>";
        }
        h += "</table>";
    }
    ...
    try { var ae = parent.document.activeElement; if (ae && ae.tagName == "SELECT" && panel.contains(ae)) return; } catch (e) {}
...
function dup_keep(name) { // Zielitem: eine Kopie mehr als Versuchsmaterial
...
async function sell_duplicates() { // im Laden stehen
    ...
    for (var i = 0; i < idx.length; i++) { var it = character.items[idx[i]]; if (!it) continue; names.push(it.name + "+" + (it.level || 0)); try { sell(idx[i], 1); } catch (e) {} await sleep(300); }
...
async function tidy_inventory() {
    if (busy || upgrading || paused || (!tidy_force && (character.esize >= INV_MIN_FREE || Date.now() < tidy_next))) return;
    busy = true;
    var free_before = character.esize, target_free = tidy_force ? 99 : INV_TARGET_FREE;
    try {
        // 1. verkaufen
        var junk = [];
        ...
        // 2. Schmuck-Dreiergruppen compounden, wenn wir eh unterwegs sind
        if (character.esize < target_free && has_compound_triples()) { try { await compound_spares(); } catch (e) { if (e == "PAUSE") throw e; } }
    ...
    if (character.esize < INV_MIN_FREE) { tidy_next = Date.now() + 5 * 60000; if (character.esize <= free_before) game_log("Inventar bleibt voll (" + character.esize + " frei) – nächster Versuch in 5 min"); }
    busy = false;
}
...
async function bank_cleanup() { // muss in der Bank stehen (oder läuft hin)
    ...
    try {
        ...
        while (rounds++ < 6) {
            ...
            set_message("Verkaufen"); await travel_place("potions");
            var sold = 0;
            for (var k = 0; k < character.items.length; k++) { var jt = character.items[k]; if (jt && !worth_keeping(jt) && !equipped_names()[jt.name]) { sell(k, jt.q || 1); sold++; await sleep(300); } }
...
async function check_monsterhunt() {
    ...
    if (q && q.c > 0) {
        if (!hunt_target_ok(q.id)) { if (hunt_spot) { game_log("Jagd auf " + q.id + " nicht sicher/gesperrt – läuft aus, normal farmen"); hunt_reset(0); hunt_cooldown_until = Date.now() + (q.ms || 1800000); } return; } // unsicher -> läuft ab, normal weiterfarmen
    ...
    try {
        ...
        if (q2 && q2.c > 0) {
            if (hunt_target_ok(q2.id)) { hunt_bad_tries = 0; game_log("Neue Jagd: " + q2.c + "x " + q2.id + " (" + fmt_time(q2.ms || 0) + ")"); }
                ...
                hunt_bad_tries++;
                game_log("Jagd auf " + q2.id + " ist nicht sicher – gebe sie auf");
...
async function check_ponty(force) {
    ...
    try {
        ...
        for (var wp = 0; wp < offer.length; wp++) { var po = offer[wp], pdef = G.items[po.name]; if (!pdef) continue; var pprice = po.price || Math.round(pdef.g * Math.pow(2, po.level || 0) * 3); if (!wish_wants(po.name, po.level, pprice)) continue; await wish_buy_from_offer({ name: po.name, level: po.level, price: pprice }, async function () { parent.socket.emit("sbuy", { rid: po.rid }); return true; }); }
        var pslots = {};
        for (var i = 0; i < offer.length; i++) {
            ...
            if (!on_wishlist(o.name) && (auto_mode || price > NONWISH_MAX_PRICE)) continue; // außerhalb des Zielbaus: nur manuell und nur günstig
...
async function check_market(force) {
    ...
    try {
        ...
        for (var w0 = 0; w0 < offers.length; w0++) { var wo = offers[w0]; if (!wish_wants(wo.name, wo.level, wo.price)) continue; if (distance(character, wo.seller) > 300) { try { await smart_move({ x: wo.seller.x, y: wo.seller.y + 30 }); } catch (e) { continue; } } await wish_buy_from_offer(wo, async function () { trade_buy(wo.seller, wo.slot, 1); return true; }); }
        ...
        for (var i = 0; i < offers.length; i++) {
            ...
            if (!on_wishlist(o.name) && (auto_mode || o.price > NONWISH_MAX_PRICE)) continue;
            var cur = character.slots[slot], cur_s = cur ? item_score(cur) : 0;
...
function compute_goals() {
    ...
    for (var slot in SLOT_TYPES) {
        ...
        if (auto_mode && !is_wish) armor_cap = 0; // Automatik: nur Zielbau-Teile werden ausgebaut, kein Gold in Zwischenlösungen
...
function rebuild_wishlist() {
    WISHLIST = {};
    for (var sl in DEFAULT_WISHLIST) WISHLIST[sl] = DEFAULT_WISHLIST[sl].slice();
    for (var sl2 in SLOT_TYPES) { var c = wish_cfg[sl2]; if (c && c.item && G.items[c.item]) WISHLIST[sl2] = [c.item]; else if (c && c.item === "") delete WISHLIST[sl2]; }
}
function default_target_level(def) { return def.compound ? COMPOUND_TARGET : (def.upgrade ? AUTO_ARMOR_MAX : 0); }
...
function slot_candidates(slot) { // alle Items, die in den Slot passen (Klasse beachtet), nach Wert auf Zielstufe sortiert
    var out = [];
    for (var n in G.items) { var d = G.items[n]; if (!d || d.ignore || !d.type || !fits_slot(d, slot)) continue; if (!d.upgrade && !d.compound && !d.int && !d.attack) continue; out.push({ name: n, def: d, score: gear_score(d, default_target_level(d)) }); }
...
function item_tooltip(n) { var d = G.items[n]; return (d.name || n) + " [" + n + "]\nBasis: " + (stat_line(d) || "-") + "\nje Stufe (" + (d.compound ? "Compound" : "Upgrade") + "): " + (stat_line(d.upgrade || d.compound) || "-") + "\nQuelle: " + item_source(n) + "\nWert +0: " + Math.round(gear_score(d, 0)) + ", auf Zielstufe: " + Math.round(gear_score(d, default_target_level(d))); }
...
function build_vs_buy(slot) {
    ...
    if (have >= tl) { build.cost = 0; build.note = "Ziel erreicht"; }
        ...
        if (have < 0) { if (is_buyable(item)) build.cost += base_price(def, item); else { build.possible = false; build.note = "Basis fehlt (nur Drop/Markt)"; } }
    ...
    var offers = global_offers.filter(function (o) { return o.name == item && o.price <= Math.max(0, wish_budget()); });
    var best = null;
    ...
    else if (!best) verdict = build.possible ? "bauen (kein Angebot)" : "warten (kein Angebot, keine Basis)";
    else if (!build.possible) verdict = "kaufen";
...
function bvb_html(slot) {
    ...
    h += "<div style='color:#9aa3b2'>bauen: " + (b.from >= r.target ? "–" : (b.possible ? "~" + fmt(b.cost) + (b.from >= 0 ? " ab +" + b.from : " ab Kauf") + (b.p < 0.999 ? ", überlebt " + Math.round(b.p * 100) + " %" : "") : b.note)) + "</div>";
...
function wish_ui_html() { // Zielbau-Tabelle
    var h = "<table class='lp_t'><tr><th style='text-align:left'>Slot</th><th style='text-align:left'>Zielitem (Tooltip = Werte)</th><th>Stufe</th><th style='text-align:left'>Stand</th><th style='text-align:left'>Bauen oder kaufen?</th><th style='text-align:left'>Angebote alle Server (★ = hier)</th><th></th></tr>";
    for (var slot in SLOT_TYPES) {
        ...
        cands.forEach(function (c) { sel += "<option value='" + c.name + "'" + (c.name == cur ? " selected" : "") + " title='" + esc(item_tooltip(c.name)) + "'>" + esc(c.def.name || c.name) + " (" + Math.round(c.score) + ")</option>"; });
        sel += "</select>";
        var lv = "";
        ...
        if (cur) { var ok = worn && worn.name == cur, done = ok && (worn.level || 0) >= wish_level(slot); stand = "<span style='color:" + (done ? "#4caf50" : ok ? "#8ab4f8" : "#e57373") + "' title='" + esc(cur + " – " + item_tooltip(cur)) + "'>" + stand + (done ? " ✓" : ok ? " → +" + wish_level(slot) : " → " + esc(cur)) + "</span>"; }
        h += "<tr><td style='text-align:left'>" + slot + "</td><td style='text-align:left'>" + sel + "</td><td>" + lv + "</td><td style='text-align:left'>" + stand + "</td><td style='text-align:left;white-space:normal;max-width:200px;font-size:11px'>" + (cur ? bvb_html(slot) : "-") + "</td><td style='text-align:left;white-space:normal;max-width:190px'>" + offers_html(slot) + "</td><td><button data-act='focus' data-slot='" + slot + "' title='diesen Slot jetzt angehen'>Jetzt</button></td></tr>";
    }
    return h + "</table>";
}
...
var AUTO_GEAR = true, AUTO_GEAR_INTERVAL = 30 * 60000, last_auto_gear = 0, WEAPON_SAFE_TARGET = 7, AUTO_ARMOR_MAX = 7;
...
function wish_wants(name, level, price) { // Slot, für den dieses Angebot die Wunschliste verbessert – sonst null
    var def = G.items[name]; if (!def || price == null) return null;
    for (var slot in WISHLIST) {
        var r = wish_rank(slot, name); if (r < 0) continue;
        var tl = wish_level(slot), own = 0; try { own = cost_to_reach(name, Math.max(tl, 1)) * 1.5; } catch (e) {}
        var cap = Math.max(WISH_MAX[name] || 0, def.g * 8, (level || 0) >= tl ? own : 0); // fertig auf Zielstufe darf so viel kosten wie der eigene Ausbau (inkl. Risiko)
        if (price > cap || price > wish_budget()) continue;
...
async function focus_slot(slot) { // Button "Jetzt": diesen Slot sofort verbessern – Wunschliste am Markt/Ponty, sonst getragenes Teil ausbauen
    ...
    upgrading = true; busy = true; set_message(slot);
    ...
    try {
        ...
        if (WISHLIST[slot] && r0 < WISHLIST[slot].length) {
            ...
            var bvb = build_vs_buy(slot); if (bvb) game_log("Bauen oder kaufen (" + bvb.item + " +" + bvb.target + "): " + bvb.verdict + " – bauen ~" + fmt(bvb.build.cost) + (bvb.build.p < 0.999 ? " (überlebt " + Math.round(bvb.build.p * 100) + " %)" : "") + ", kaufen " + (bvb.buy ? "~" + fmt(bvb.buy.total) + " auf " + pretty_server(bvb.buy.server) : "kein Angebot"));
    ...
    GOLD_RESERVE = saved_reserve; upgrading = false; busy = false;
    after_action("Slot");
}
...
function offers_for_slot(slot) { // beste Angebote aller Server für einen Slot: Wunschliste zuerst, dann nach Wert je Gold; nur besser als getragen und bezahlbar
    ...
    var list = global_offers.filter(function (o) { var def = G.items[o.name]; return def && fits_slot(def, slot) && o.score > ws && o.price <= Math.max(wish_budget(), 0); });
...
function offers_html(slot) {
    ...
    return list.map(function (o) { return "<span style='color:" + (o.same ? "#4caf50" : (o.wish > 0 ? "#8ab4f8" : "#e6e6e6")) + "' title='" + esc((o.wish > 0 ? "Wunschliste · " : "Alternative · ") + "Wert " + Math.round(o.score) + " · Händler " + o.seller + " auf " + o.server + (o.same ? " (dieser Server)" : "")) + "'>" + esc(o.name) + "+" + o.level + " " + fmt(o.price) + " <small>" + esc(pretty_server(o.server)) + (o.same ? " ★" : "") + "</small></span>"; }).join("<br>");
}
...
function server_tip_html() {
    ...
    return list.slice(0, 3).map(function (f) { return "<span style='color:#8ab4f8'>→ <b>" + esc(pretty_server(f.server)) + "</b>: " + esc(f.name) + "+" + f.level + " für " + fmt(f.price) + " (" + esc(f.seller) + ")</span>"; }).join(" · ") + " <span style='color:#9aa3b2'>– nach dem Wechsel kauft der Bot von selbst</span>";
}
function check_server_arrival() { // nach Serverwechsel: liegt hier ein gemerkter Fund? -> sofort kaufen
    ...
    var here = server_targets.filter(function (f) { return Date.now() - f.t < 45 * 60000 && norm_server(f.server) == mine && wish_wants(f.name, f.level, f.price); });
...
async function scan_all_merchants(force, only_slot) {
    ...
    global_finds = finds; global_offers = all; try { parent.__lp_global_offers = all; } catch (e) {}
...
async function buy_find(f) { // Treffer auf eigenem Server sofort kaufen (innerhalb einer laufenden Routine)
    if (!f || !wish_wants(f.name, f.level, f.price)) return false;
...
async function run_pending_buy() { // zum Händler auf diesem Server laufen und kaufen
    ...
    if (!wish_wants(f.name, f.level, f.price)) return;
...
async function upgrade_routine_inner(manual) {
    ...
    var weapon_todo = character.slots.mainhand && G.items[character.slots.mainhand.name].upgrade && (manual || wish_item("mainhand") == character.slots.mainhand.name) && (character.slots.mainhand.level || 0) < slot_target("mainhand", is_buyable(character.slots.mainhand.name) ? UPGRADE_TARGET : WEAPON_SAFE_TARGET, 0);
    ...
    try {
        ...
        if (mh && G.items[mh.name].upgrade && (manual || wish_item("mainhand") == mh.name)) { check_pause(); await process_slot("mainhand", slot_target("mainhand", is_buyable(mh.name) ? UPGRADE_TARGET : WEAPON_SAFE_TARGET, 0)); }
        // 2. Rest nach Wert je Aufwand
        await upgrade_by_recommendation();

        stat_slots = spendable() >= stat_price ? slots_without_stat().filter(function (sl) { return manual || wish_item(sl) == character.slots[sl].name; }) : [];
...
function start_main() {
  ...
  main_timer = parent.__lp_main_timer = setInterval(function () {
    heal_logic(); loot();
    if (character.rip) { if (!rip_counted) { rip_counted = true; day_count("deaths"); game_log("Gestorben (heute " + day.deaths + "x)"); } if (meas) finish_measure(true); respawn(); busy = false; fleeing = false; kissing = false; return; }
    rip_counted = false;
    session_tick();
    ...
    busy_watchdog(); auto_gear_tick(); best_equip_tick(); scan_all_merchants(); run_pending_buy(); run_goal(); check_weapon(); check_gear_slots(); check_flee(); check_elixir(); kiss_routine(); tidy_inventory(); check_potions(); check_stuck(); check_ponty(false); check_market(false); check_seashells(); check_monsterhunt(); check_cake();
...
rebuild_wishlist();
setTimeout(function () { try { check_server_arrival(); } catch (e) {} }, 4000);

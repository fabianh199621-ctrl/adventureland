...
function best_equip_tick() { if (busy || upgrading || paused || Date.now() - last_best_check < 5 * 60000) return; last_best_check = Date.now(); ensure_best_equipped().catch(function () {}); }
...
function start_main() {
  ...
  main_timer = parent.__lp_main_timer = setInterval(function () {
    ...
    busy_watchdog(); auto_gear_tick(); best_equip_tick(); run_goal(); check_weapon(); check_gear_slots(); check_flee(); check_elixir(); kiss_routine(); tidy_inventory(); check_potions(); check_stuck(); check_ponty(false); check_market(false); check_seashells(); check_monsterhunt(); check_cake();

...
async function go_tidy() { // Aufräumen auf Befehl des Magiers: Schrott verkaufen, Rest in die Bank, Tränke und Tokens behalten
    ...
    try {
        ...
        var failed = 0;
        var last_err = "";
        if (bank.length) { try { await smart_move("bank"); for (var w2 = 0; w2 < 25 && !character.bank; w2++) await sleep(200); if (!character.bank) throw "Bankdaten nicht geladen (Karte " + character.map + ")";

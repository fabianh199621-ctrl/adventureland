...
async function go_tidy() { // Aufräumen auf Befehl des Magiers: Schrott verkaufen, Rest in die Bank, Tränke und Tokens behalten
    ...
    try {
        ...
        var stored = 0;
        if (bank.length) { try { await smart_move("bank"); await sleep(800); for (var b = bank.length - 1; b >= 0; b--) { var ib = character.items[bank[b]]; if (!ib) continue; try { bank_store(bank[b]); stored++; } catch (e) {} await sleep(350); } } catch (e) { say("Bank: " + (e && e.message || e)); } }
        say("Aufgeräumt: " + sold + " verkauft, " + stored + " in die Bank – frei " + character.esize);

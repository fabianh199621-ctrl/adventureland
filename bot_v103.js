...
function init_panel() {
    ...
    var onClick = function (e) {
        ...
        if (act == "farm") { user_manual = mon; set_manual_spot(mon); } else if (act == "auto") set_auto_spot(); else if (act == "reset") reset_measurements();
...
function has_compound_triples() {
    var g = {};
    for (var i = 0; i < character.items.length; i++) { var it = character.items[i]; if (!it || it.p || it.stat_type) continue; var def = G.items[it.name]; if (!def || !def.compound || (it.level || 0) >= COMPOUND_SPARE_MAX) continue; var k = it.name + "/" + (it.level || 0); g[k] = (g[k] || 0) + 1; if (g[k] >= 3) return true; }
    return false;
}
...
async function bank_compound_jewelry() { // muss in der Bank stehen
    for (var round = 0; round < 4; round++) {
        ...
        var tally = function (it) { var def = G.items[it.name]; if (!def || !def.compound || it.p || it.stat_type || (it.level || 0) >= COMPOUND_SPARE_MAX) return null; var k = it.name + "/" + (it.level || 0); cnt[k] = (cnt[k] || 0) + 1; return k; };
...
async function compound_spares() {
    ...
    character.items.forEach(function (it) {
        if (!it || it.p || it.stat_type) return; var def = G.items[it.name]; if (!def || !def.compound) return;
...
function start_main() {
  ...
  main_timer = parent.__lp_main_timer = setInterval(function () {
    ...
    busy_watchdog(); run_goal(); check_weapon(); check_gear_slots(); check_flee(); check_elixir(); kiss_routine(); tidy_inventory(); check_potions(); check_stuck(); check_ponty(false); check_market(false); check_seashells(); check_monsterhunt(); check_cake();

...
var KEEP_ITEMS = /^(hpot|mpot|elixirint|scroll|cscroll|intscroll|strscroll|dexscroll|vitscroll|tracker|seashell)/;
...
function init_panel() {
    ...
    var onMove = function (e) { if (!drag) return; drag.el.style.left = (e.clientX - drag.dx) + "px"; drag.el.style.top = (e.clientY - drag.dy) + "px"; e.preventDefault(); e.stopPropagation(); };
...
function init_char_panel() {
    ...
    try { var p = JSON.parse(localStorage.getItem("lp_char_pos") || "null"); if (p) { div.style.left = p.x + "px"; div.style.top = p.y + "px"; } } catch (e) {}
    doc.body.appendChild(div);
    return div;
}
...
async function check_ponty(force) {
    ...
    try {
        ...
        for (var i = 0; i < offer.length; i++) {
            ...
            if ((cur && cur.name == o.name && (cur.level || 0) >= (o.level || 0)) || locate_item(o.name) >= 0) continue;
...
async function check_market(force) {
    ...
    try {
        ...
        for (var i = 0; i < offers.length; i++) {
            ...
            var cur = character.slots[slot], cur_s = cur ? item_score(cur) : 0;
            if ((cur && cur.name == o.name && (cur.level || 0) >= (o.level || 0)) || locate_item(o.name) >= 0) continue; // schon vorhanden
            goal_cache_t = 0; compute_goals();
            var is_goal = goal_cache[slot] && goal_cache[slot].want_market == o.name;
...
function start_main() {
  ...
  main_timer = parent.__lp_main_timer = setInterval(function () {
    ...
    run_goal(); check_weapon(); check_gear_slots(); check_flee(); check_elixir(); kiss_routine(); tidy_inventory(); check_potions(); check_stuck(); check_ponty(false); check_market(false); check_seashells();

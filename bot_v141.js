...
function team_tick() { // fehlende Teammitglieder starten, abgeschaltete stoppen; Party pflegen
    ...
    for (var k in TEAM) {
        ...
        if (on && !running) { if (Date.now() - (team_start_at[nm] || 0) > 90000) { team_start_at[nm] = Date.now(); try { start_character(nm, team_slot()); game_log("Team: starte " + nm + " (Code-Slot " + team_slot() + ")"); } catch (e) { game_log("Team: " + nm + " konnte nicht gestartet werden – " + err_txt(e)); } } }
...
function team_broadcast() { // alle 5 s: wo bin ich, was mache ich (für Händler und Priester)
    ...
    var msg = { t: "me", map: character.map, x: Math.round(character.x), y: Math.round(character.y), level: character.level, hp: character.hp, max_hp: character.max_hp, paused: paused || !bot_running, spot: current_spot, tgt: last_target_id, mluck: ml ? { f: ml.f, ms: ml.ms } : null, in: character.in };
...
function on_cm(name, data) { // Nachrichten der eigenen Charaktere
    ...
    try {
        ...
        else if (data.t == "hello") { team_send(name, { t: "state", paused: paused || !bot_running, spot: current_spot }); team_broadcast(); }
...
function reload_bot() {
    ...
    fetch(BOT_BASE + "version.txt?t=" + Date.now(), { cache: "no-store" })
        ...
        .then(function (code) {
            ...
            try { var ac = active_chars(); TEAM_NAMES.forEach(function (nm) { if (ac[nm]) { try { stop_character(nm); } catch (e) {} } }); } catch (e) {} // Team lädt danach ebenfalls die neue Version
...
function start_main() {
  ...
  main_timer = parent.__lp_main_timer = setInterval(function () {
    ...
    try { team_tick(); team_broadcast(); } catch (e) {}
    if (paused) return;
    measure_tick();

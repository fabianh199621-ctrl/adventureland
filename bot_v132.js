...
function wish_ui_html() { // Zielbau-Tabelle
    ...
    for (var slot in SLOT_TYPES) {
        ...
        if (cur) { var ok = worn && worn.name == cur, done = ok && (worn.level || 0) >= wish_level(slot); stand = "<span style='color:" + (done ? "#4caf50" : ok ? "#8ab4f8" : "#e57373") + "' title='" + esc(cur + " – " + item_tooltip(cur)) + "'>" + stand + (done ? " ✓" + (wish_cfg[slot] && wish_cfg[slot].auto ? " <small>(Ist-Stand)</small>" : "") : ok ? " → +" + wish_level(slot) : " → " + esc(cur)) + "</span>"; }

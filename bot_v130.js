...
function check_trip_on_start() { // nach dem Laden: sind wir unterwegs?
    ...
    if (server_trip.stage == "out" && mine == norm_server(server_trip.target.region + server_trip.target.id)) {
        ...
        if (!wish_wants(f.name, f.level, f.price)) { end_trip("Angebot passt nicht mehr"); return; }
...
async function scan_all_merchants(force, only_slot) {
    ...
    last_panel = 0;
    if (auto_trip && !here && !server_trip && !busy && !upgrading && !paused) { var tip = server_tip()[0]; if (tip) { game_log("Serverkauf automatisch: " + tip.name + "+" + tip.level + " für " + fmt(tip.price) + " auf " + pretty_server(tip.server)); start_trip(tip); } }

...
function team_tick() { // fehlende Teammitglieder starten, abgeschaltete stoppen; Party pflegen
    ...
    for (var k in TEAM) {
        ...
        if (on && !running) { if (Date.now() - (team_start_at[nm] || 0) > 60000) { team_start_at[nm] = Date.now(); try { var res = start_character(nm, team_slot()); game_log("Team: starte " + nm + " (Code-Slot " + team_slot() + ") – aktiv: " + JSON.stringify(act)); if (res && typeof res.then == "function") res.then(function (r) { game_log("Team: " + nm + " Start-Antwort: " + JSON.stringify(r).slice(0, 120)); }, function (e) { game_log("Team: " + nm + " Start abgelehnt: " + JSON.stringify(e).slice(0, 160)); }); } catch (e) { game_log("Team: " + nm + " konnte nicht gestartet werden – " + err_txt(e)); } } }

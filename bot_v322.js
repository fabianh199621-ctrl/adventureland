...
function priority_mon(mtype) { if (PRIORITY_MONS[mtype]) return true; if (event_mode && mtype == event_mode) return true; var d = G.monsters[mtype]; return !!d && d.cooperative && (d.attack || 0) * 8 < character.max_hp && !d.boss; } // kooperative Spawns mit schwachem Angriff ebenfalls
...
function is_valid_target(m) {
    ...
    if (event_mode && m.mtype == event_mode && EVENTS[event_mode]) return true; // Event-Boss ist immer ein gültiges Ziel
...
var merch_needs = {}, merch_need_q = {}, last_need_buy = 0, merch_need_logged = 0; // Material, das der Händler angefragt hat (z. B. spidersilk für Werkzeuge) + Menge
try { var mn0 = JSON.parse(localStorage.getItem("lp_merch_needs") || "null"); if (mn0) { merch_needs = mn0.n || {}; merch_need_q = mn0.q || {}; } } catch (e) {} // bleibt über Neuladen erhalten
function merch_needs_save() { try { localStorage.setItem("lp_merch_needs", JSON.stringify({ n: merch_needs, q: merch_need_q })); } catch (e) {} }
function merch_need_offer(name) { // günstigstes Marktangebot für angefragtes Material unter dem Höchstpreis (eigener Server bevorzugt)
    var cap = MG.mat_max || 0; if (!(cap > 0)) return null; var best = null;
    (market_all || []).forEach(function (o) { if (o.b || o.name != name || o.price > cap || is_pvp_server(o.server)) return; if (!best || (o.same && !best.same) || (o.same == best.same && o.price < best.price)) best = o; });
    return best;
}
function merch_need_tick() { // alle 2 min: Material, das der Händler braucht, auf dem Markt kaufen (er zahlt aus seiner Kasse, behält es selbst)
    if (SOLO || Date.now() - last_need_buy < 120000 || merch_buy || arb_job) return; last_need_buy = Date.now();
    for (var nm in merch_needs) {
        if (Date.now() - merch_needs[nm] > 24 * 3600000) { delete merch_needs[nm]; merch_needs_save(); continue; }
        if (locate_item(nm) >= 0) continue; // habe ich selbst – geht bei der nächsten Abholung mit
        var o = merch_need_offer(nm); if (!o) { if (Date.now() - (merch_need_logged || 0) > 15 * 60000) { merch_need_logged = Date.now(); var chp = null; (market_all || []).forEach(function (x) { if (!x.b && x.name == nm && (!chp || x.price < chp.price)) chp = x; }); game_log("Händler-Bedarf " + nm + ": kein Angebot bis " + fmt(MG.mat_max || 0) + (chp ? " – günstigstes " + fmt(chp.price) + " bei " + chp.seller + " (" + pretty_server(chp.server) + ")" : " – im letzten Scan gar nicht angeboten")); } continue; }
        var want = Math.max(1, Math.min(merch_need_q[nm] || 1, o.q || 1));
        if (merch_buy_start(Object.assign({}, o, { want: want }), o.same ? "here" : "trip")) { merch_buy.for_merch = true; merch_buy.o.for_merch = true; mb_save(); game_log("Händler-Bedarf: " + want + "× " + nm + " für " + fmt(o.price) + " je Stück bei " + o.seller + " (" + pretty_server(o.server) + ") – Händler kauft es selbst und behält es"); return; }
    }
}
function on_cm(name, data) { // Nachrichten der eigenen Charaktere
    ...
    try {
        ...
        else if (data.t == "npcsold") { delete npc_orders[data.key]; npc_orders_save(); game_log("[" + who + "] NPC-Verkauf: " + data.q + "× " + data.name + (data.level ? "+" + data.level : "") + " für " + fmt(data.gold) + " Gold (liegt in der Händlerkasse)"); if (bank_panel) render_bank(); }
        ...
        else if (data.t == "need") { var np = get_player(name); var ni = locate_item(data.item); if (np && ni >= 0 && distance(character, np) < 400) { var nq = Math.min(data.q || 1, character.items[ni].q || 1); try { send_item(name, ni, nq); game_log("[" + who + "] " + nq + "x " + data.item + " übergeben"); } catch (e) {} } else if (ni < 0) { merch_needs[data.item] = Date.now(); merch_need_q[data.item] = data.q || 1; merch_needs_save(); last_need_buy = 0; game_log("[" + who + "] braucht " + data.item + " – wird beim nächsten Fund/Abholung mitgegeben" + (merch_need_offer(data.item) ? ", oder er kauft es selbst auf dem Markt" : "")); } }
...
function event_tick() {
    ...
    if (event_mode == "goobrawl" || event_mode == "abtesting") return; // Goo-Prügelei/PvP haben ihre eigene Steuerung (goo_tick/ab_tick)
    var st = event_status();
    if (!event_mode) {
...
// ---------- Event-Bosse mit Vorrang: Liste nach Priorität; ein höheres Event unterbricht ein laufendes niedrigeres ----------
var EVENTS = { // prio: kleiner = wichtiger. join: Teleport-Event. needPriest: nur mit Priest daneben.
    goobrawl: { label: "Goo-Prügelei", prio: 1, join: true },
    crabxx: { label: "Giga Crab", prio: 2, join: true },
    dragold: { label: "Dragold", prio: 3, needPriest: true },
    grinch: { label: "Grinch", prio: 4 },
    wabbit: { label: "Wabbit", prio: 5 },
    snowman: { label: "Snowman", prio: 6 },
    pinkgoo: { label: "Love Goo", prio: 7 },
    abtesting: { label: "A/B-Test (PvP)", prio: 8, join: true, pvp: true } // sicheres Team-PvP: Mage + Ranger springen rein, Priest bleibt draußen
};
function ev_away() { return event_mode == "goobrawl" || event_mode == "abtesting"; } // Teleport-Event: keine Stadtgänge/Käufe vom Event-Ort
...
function ev_leave_current(why) { if (event_mode == "goobrawl") goo_leave(why); else if (event_mode == "abtesting") ab_leave(why); else if (event_mode == "crabxx") event_leave(why); else ev_leave(why); }
var last_ev_check = 0, ev_join_t = 0;
function ev_tick() { // Bosse an fester Position (nicht goobrawl/crabxx – die haben eigene Steuerung, aber die Priorität gilt für alle)
    if (Date.now() - last_ev_check < 10000) return; last_ev_check = Date.now();
    var best = ev_best();
    if (event_mode && EVENTS[event_mode] && best && best != event_mode && EVENTS[best].prio < EVENTS[event_mode].prio) { ev_leave_current("wichtigeres Event: " + EVENTS[best].label); return; } // Vorrang: laufendes Event verlassen, nächster Tick startet das wichtigere
    if (event_mode) { if (EVENTS[event_mode] && !EVENTS[event_mode].join) { if (!ev_status(event_mode)) { ev_leave(EVENTS[event_mode].label + " vorbei"); } else if (!ev_enabled(event_mode)) ev_leave("abgeschaltet"); } return; }
    if (!best || EVENTS[best].join) return; // Teleport-Events starten goo_tick/event_tick
    if (paused || busy || upgrading || server_trip || focus_mode || arb_job || merch_test || kissing || handing || character.rip) return;
    event_prev = { manual_spot: manual_spot, user_manual: user_manual }; event_mode = best; event_since = Date.now();
    manual_spot = best; user_manual = null; current_spot = best; need_repick = true; meas = null; if (hunt_spot) hunt_spot = null;
    var st = ev_status(best); game_log("Event: " + EVENTS[best].label + " ist da (" + (st && st.map ? st.map + " " + Math.round(st.x || 0) + "," + Math.round(st.y || 0) : "Position unbekannt") + (st && st.max_hp ? ", HP " + Math.round((st.hp || 0) / st.max_hp * 100) + " %" : "") + ") – Team geht hin, Jagd/Farmen wartet"); last_panel = 0;
    stop("smart"); change_target(null); last_go = 0; go_to_farm_spot();
}
function ev_leave(why) {
    game_log("Event: " + why + " – zurück zum normalen Betrieb");
...
// ---------- A/B-Test (Server-Event abtesting): sicheres Team-PvP, Beitritt nur in den ersten 2 Minuten; Mage + Ranger rein, Priest farmt weiter ----------
var last_ab_check = 0, ab_join_t = 0, ab_fail = 0, ab_skip_until = 0, ab_hits = 0, ab_roam_t = 0;
function ab_tick() {
    if (Date.now() - last_ab_check < 10000) return; last_ab_check = Date.now();
    var st = ev_status("abtesting");
    if (event_mode != "abtesting") {
        if (!ev_enabled("abtesting") || !st || event_mode || paused || busy || upgrading || server_trip || focus_mode || arb_job || merch_test || kissing || handing || character.rip || Date.now() < ab_skip_until) return;
        if (ev_best() != "abtesting") return; // wichtigeres Event läuft gleichzeitig
        event_prev = { manual_spot: manual_spot, user_manual: user_manual }; event_mode = "abtesting"; event_since = Date.now(); ab_fail = 0; ab_hits = 0;
        game_log("A/B-Test (PvP) aktiv – Mage und Ranger springen rein (Priest farmt weiter; kein Risiko, kleine Belohnung)"); last_panel = 0;
        ab_join(); return;
    }
    if (!st) { ab_leave("Event vorbei" + (ab_hits ? " (" + ab_hits + " Treffer)" : "")); return; }
    if (!ev_enabled("abtesting")) { ab_leave("abgeschaltet"); return; }
    if (character.map != "abtesting" && !busy && Date.now() - ab_join_t > 30000) { if (ab_fail >= 2) { ab_skip_until = Date.now() + 20 * 60000; ab_leave("Beitritt nicht möglich (Fenster zu?) – dieses Mal auslassen"); return; } ab_join(); }
}
function ab_join() {
    ab_join_t = Date.now();
    try { var r = join("abtesting"); if (r && typeof r.then == "function") r.then(function () { game_log("A/B-Test: angekommen (Team " + (character.team || "?") + ")"); ab_fail = 0; }, function (e) { ab_fail++; game_log("A/B-Test: Sprung fehlgeschlagen: " + JSON.stringify(e).slice(0, 120)); }); } catch (e) { ab_fail++; game_log("A/B-Test: join-Fehler " + err_txt(e)); }
    if (team_on.ranger && team_running(TEAM.ranger)) team_send(TEAM.ranger, { t: "join", event: "abtesting" });
}
function ab_leave(why) {
    game_log("A/B-Test: " + why + " – zurück zum normalen Betrieb");
    if (event_prev) { manual_spot = event_prev.manual_spot; user_manual = event_prev.user_manual; } event_prev = null; event_mode = null;
    current_spot = null; need_repick = true; meas = null; last_hunt_check = 0; last_panel = 0;
    if (team_on.ranger && team_running(TEAM.ranger)) team_send(TEAM.ranger, { t: "town" });
    stop("smart"); change_target(null);
    if (character.map == "abtesting") { busy = true; (async function () { try { await town_teleport(); } catch (e) { try { await smart_move("town"); } catch (e2) {} } busy = false; if (!paused) go_to_farm_spot(); })(); }
    else if (!paused) go_to_farm_spot();
}
function pvp_enemy() { // nächster Spieler des anderen Teams (ohne Team-Info: alle außer den eigenen Chars)
    var best = null, bd = 1e9;
    try { for (var id in parent.entities) { var p = parent.entities[id]; if (!p || p.type != "character" || p.npc || p.rip || p.dead) continue; if (character.team) { if (!p.team || p.team == character.team) continue; } else if (TEAM_NAMES.indexOf(p.id) >= 0) continue; var d = distance(character, p); if (d < bd) { bd = d; best = p; } } } catch (e) {}
    return best;
}
function ab_step() { // in der Arena: Gegner suchen, ran, draufhauen
    var e = pvp_enemy();
    if (!e) { set_message("PvP: kein Gegner"); if (!is_moving(character) && Date.now() - ab_roam_t > 4000) { ab_roam_t = Date.now(); try { move(character.x + (Math.random() - 0.5) * 300, character.y + (Math.random() - 0.5) * 300); } catch (x) {} } return; }
    if (!is_in_range(e)) { if (!is_moving(character)) { try { move(character.x + (e.x - character.x) / 2, character.y + (e.y - character.y) / 2); } catch (x) {} } set_message("PvP: zu " + e.id); return; }
    if (can_attack(e)) { try { attack(e); ab_hits++; } catch (x) {} } set_message("PvP: " + e.id + " " + Math.round((e.hp || 0) / (e.max_hp || 1) * 100) + " %");
}
...
function event_html() { // Zeile nur, wenn gerade ein Event läuft oder wir dabei sind
    ...
    return "<div class='lp_row'><span class='lp_mode' style='color:#9aa3b2'>Event: <span style='color:#e6e6e6'>" + esc(live.join(", ") || "-") + (event_mode ? " – dabei: " + esc((EVENTS[event_mode] || {}).label || event_mode) + " seit " + Math.round((Date.now() - event_since) / 60000) + " min" + (event_mode == "crabxx" ? ", " + event_boss_hits + " Treffer" : event_mode == "abtesting" ? ", " + ab_hits + " Treffer, Team " + (character.team || "?") : "") : (events_on ? "" : " (Events aus)")) + "</span></span></div>";
}
...
var newer_check_t = 0;
function check_newer_version() { // Teamdatei nicht mehr auf GitHub (neue Version veröffentlicht, alte gelöscht): selbst neu laden
    if (Date.now() - newer_check_t < 120000) return; newer_check_t = Date.now();
    fetch(BOT_BASE + "version.txt?t=" + Date.now(), { cache: "no-store" }).then(function (r) { return r.ok ? r.text() : ""; }).then(function (v) { v = String(v).trim(); if (v && v != BOT_VERSION) { game_log("Neue Version " + v + " auf GitHub (läuft " + BOT_VERSION + ") – lade neu"); reload_bot(); } }).catch(function () {});
}
function reload_bot() {
...
function team_hunt_pick() { // Jagdmonster von Priest/Ranger, das für uns sicher ist (nur wenn keine eigene Jagd läuft)
    if (SOLO || hunt_spot || event_mode) return null; // Event hat Vorrang vor Team-Jagden
...
function go_to_farm_spot() {
    ...
    last_go = Date.now();
    if (event_mode == "goobrawl") { // Goo-Prügelei: auf der Insel bleiben, ohne Goo in Sicht innerhalb des Feldes umherstreifen
        if (character.map != "goobrawl") { if (Date.now() - goo_join_t > 30000) goo_join(); return; }
        var gt = null, gd = 1e9; try { for (var gid in parent.entities) { var gm = parent.entities[gid]; if (gm && gm.type == "monster" && !gm.dead && (gm.mtype == "rgoo" || gm.mtype == "goo" || gm.mtype == "bgoo")) { var dd = distance(character, gm); if (dd < gd) { gd = dd; gt = gm; } } } } catch (e) {}
        if (gt) { if (gd > 200) { busy = true; smart_move({ x: gt.x + 30, y: gt.y }).catch(function () {}).then(function () { busy = false; }); } return; }
        var gb = [-400, -440, 420, 190]; busy = true; set_message("Suche Goos"); smart_move({ x: gb[0] + Math.random() * (gb[2] - gb[0]), y: gb[1] + Math.random() * (gb[3] - gb[1]) }).catch(function () {}).then(function () { busy = false; });
        return;
    }
    if (event_mode == "abtesting") return; // PvP-Arena: ab_step steuert
    if (event_mode && EVENTS[event_mode] && event_mode != "crabxx") { // Event-Boss an fester Position (Grinch, Wabbit, Snowman …): hin und dranbleiben
        var evs = ev_status(event_mode), evb = ev_boss_entity(event_mode);
        var evp = evb ? { map: character.map, x: evb.x, y: evb.y } : (evs && evs.x != null ? { map: evs.map, x: evs.x, y: evs.y } : null);
        if (!evp) return;
        if (character.map != evp.map || distance(character, evp) > 900) { busy = true; set_message("Zum Event: " + EVENTS[event_mode].label); travel({ map: evp.map, x: evp.x, y: evp.y }).catch(function () {}).then(function () { busy = false; }); return; }
        if (distance(character, evp) > 250) { busy = true; var ang2 = Math.random() * Math.PI * 2; smart_move({ map: evp.map, x: evp.x + Math.cos(ang2) * 120, y: evp.y + Math.sin(ang2) * 120 }).catch(function () {}).then(function () { busy = false; }); }
        return;
    }
...
// ---------- Todes-Analyse: die letzten 12 s vor dem Tod (HP-Verlauf, Angreifer, Team-Abstände) ----------
var death_trace = [], death_trace_t = 0;
function death_trace_tick() {
    if (character.rip || Date.now() - death_trace_t < 500) return; death_trace_t = Date.now();
    var atk = []; try { for (var id in parent.entities) { var e = parent.entities[id]; if (e && e.type == "monster" && !e.dead && e.target == character.name) atk.push(e.mtype + (e.level > 1 ? "+" + e.level : "") + "@" + Math.round(distance(character, e))); } } catch (e) {}
    var team = {}; ["priest", "ranger"].forEach(function (k) { if (!team_on[k]) return; var p = null; try { p = get_player(TEAM[k]); } catch (e) {} team[k] = p ? (p.rip ? "tot" : p.map != character.map ? "andere Karte" : Math.round(distance(character, p)) + " px") : "außer Sicht"; });
    death_trace.push({ t: Date.now(), hp: character.hp, mp: character.mp, atk: atk, team: team, map: character.map, x: Math.round(character.x), y: Math.round(character.y), st: paused ? "Pause" : busy ? "unterwegs" : fleeing ? "Rückzug" : handing ? "Übergabe" : "kämpft" });
    while (death_trace.length && Date.now() - death_trace[0].t > 12000) death_trace.shift();
}
function death_report() {
    if (!death_trace.length) return "keine Daten";
    var first = death_trace[0], last = death_trace[death_trace.length - 1], mx = character.max_hp || 1;
    var hpv = death_trace.filter(function (x, i) { return i % 4 == 0; }).map(function (x) { return Math.round(x.hp / mx * 100) + "%"; }).join("→") + "→0%";
    var cnt = {}; death_trace.forEach(function (x) { x.atk.forEach(function (a) { var nm = a.split("@")[0]; cnt[nm] = Math.max(cnt[nm] || 0, 1); }); });
    var maxAtk = 0; death_trace.forEach(function (x) { maxAtk = Math.max(maxAtk, x.atk.length); });
    var heals = 0; for (var i = 1; i < death_trace.length; i++) if (death_trace[i].hp - death_trace[i - 1].hp > mx * 0.08) heals++;
    var tm = last.team, tm0 = first.team;
    return "HP " + hpv + " in " + Math.round((last.t - first.t) / 1000) + " s · max. " + maxAtk + " Angreifer gleichzeitig (" + (Object.keys(cnt).join(", ") || "keine erkannt") + ") · Heilungen/Tränke: " + heals + " · Priest: " + (tm.priest || "-") + " (davor " + (tm0.priest || "-") + ") · Ranger: " + (tm.ranger || "-") + " · Zustand: " + last.st + " · Ort " + last.map + " " + last.x + "," + last.y;
}
...
function init_panel() {
    ...
    div.innerHTML = "<div id='lp_head'><b>LogicPlan " + BOT_VERSION + "</b><span class='lp_state' id='lp_state'>läuft</span><button data-act='chartoggle' title='Charakter & Inventar'>Char</button><button data-act='wikitoggle' title='Items, Monster, NPCs nachschlagen'>Wiki</button><button data-act='mkttoggle' title='Alle Händlerangebote aller Server'>Markt</button><button data-act='banktoggle' title='Bankinhalt mit NPC- und Marktwert – ausstellen oder beim NPC verkaufen'>Bank</button><button id='lp_toggle' title='Monsterliste ein-/ausblenden'>▾</button><button id='lp_mini' title='Fenster verkleinern/vergrößern'>▭</button><button id='lp_gear' title='Einstellungen'>⚙</button><div id='lp_set'></div></div><div id='lp_body'></div>";
    ...
    var inside = function (e) { var c = parent.document.getElementById("lp_char"), w = parent.document.getElementById("lp_wiki"), l = parent.document.getElementById("lp_list"), mk = parent.document.getElementById("lp_mkt"), bk = parent.document.getElementById("lp_bank"), tc = e.target && e.target.closest ? e.target.closest(".lp_tchar") : null; return e.target && (div.contains(e.target) || (c && c.contains(e.target)) || (w && w.contains(e.target)) || (l && l.contains(e.target)) || (mk && mk.contains(e.target)) || (bk && bk.contains(e.target)) || !!tc); };
    var onDown = function (e) {
        ...
        if (head.contains(e.target)) { drag = { el: div, key: "lp_panel_pos", dx: e.clientX - div.offsetLeft, dy: e.clientY - div.offsetTop }; e.preventDefault(); }
                      ...
                      else { var bkw = parent.document.getElementById("lp_bank"), bkh = bkw && bkw.querySelector("#lp_bank_head"); if (bkh && bkh.contains(e.target)) { drag = { el: bkw, key: "lp_bank_pos", dx: e.clientX - bkw.offsetLeft, dy: e.clientY - bkw.offsetTop }; e.preventDefault(); }
...
function is_junk(it) { // kaufbare Standardausrüstung ohne Level/Attribut; ungetragener kaufbarer Schmuck +0 in Einzelstücken; fremde Elixiere; HP-Schmuck
    ...
    if (is_team_wish_item(it) || build_protected(it) || NEVER_SELL.test(it.name)) return false; // Team-Zielbau-Teile, Bauauftrag und geschützte Items bleiben
...
function spare_for_priest(key) { // [{i, slot, gain}] – Inventarteile, die der Magier entbehren kann und die das Teammitglied verbessern
    ...
    slots.forEach(function (slot) {
        ...
        for (var i = 0; i < character.items.length; i++) {
            ...
            if (KEEP_ITEMS.test(it.name) || EVENT_ITEMS.test(it.name) || is_team_wish_item(it) || build_protected(it)) continue;
...
async function kiss_routine() {
    if (SOLO || kissing || upgrading || fleeing || busy || marketing || server_trip || ev_away() || !kiss_round_open()) return; // nicht während Kauf/Routine/Serverreise
...
function arb_bad(o) { try { var bad = JSON.parse(localStorage.getItem("lp_arb_bad_" + TEAM.merch) || "{}"); return (bad[o.name + "+" + (o.level || 0) + "@" + o.seller + "@" + o.price] || 0) > Date.now(); } catch (e) { return false; } } // vom Händler als nicht kaufbar gemeldet (24 h)
function arb_list() { return arb_offers.filter(function (o) { return !arb_bad(o) && o.profit >= ARB_MIN_PROFIT && o.profit >= o.price * ARB_MIN_MARGIN && (o.same || !is_pvp_server(o.server)); }).sort(function (a, b) { return (b.same - a.same) || (b.profit - a.profit); }); }
function gather_html() { // Händler-Sammeln: Bergbau/Angeln – Zähler, Werkzeuge, nächster Versuch (aus der Händler-Statusmeldung)
    var st = team_state[TEAM.merch], g = st && st.gather; if (!team_on.merch) return "";
    if (!g) return "<div class='lp_row'><span class='lp_k'>Sammeln:</span> <span class='lp_k'>keine Meldung vom Händler</span></div>";
    var one = function (k, lab) { var x = g.stats && g.stats[k] || { n: 0, gold: 0, items: 0, last: 0 }; return lab + " " + x.n + "×" + (x.n ? " (+" + fmt(x.gold) + " Gold, " + x.items + " Items" + (x.last ? ", zuletzt vor " + fmt_time(Date.now() - x.last) : "") + ")" : "") + " · nächster: " + esc(g.next && g.next[k] || "?"); };
    var tools = "Spitzhacke " + (g.tools && g.tools.pickaxe ? "✓" : "✗") + " · Rute " + (g.tools && g.tools.rod ? "✓" : "✗");
    return "<div class='lp_row' title='Der Händler baut ab bzw. angelt, sobald der Skill-Cooldown frei ist und nichts Wichtigeres ansteht (Abholung, Reise, Stand-Auftrag). Ertrag geht beim Aufräumen in Bank/Stand.'><span class='lp_k'>Sammeln:</span> <span style='font-size:11px'>" + one("mining", "Bergbau") + " &nbsp;|&nbsp; " + one("fishing", "Angeln") + " &nbsp;|&nbsp; " + tools + "</span></div>";
}
...
function market_note_scan(merchants) { // aus dem Rohscan: komplette Angebotsliste + Preisgeschichte
    ...
    merchants.forEach(function (m) {
        for (var sl in m.slots) {
            ...
            all.push(o);
            if (!o.b) { var k = o.name + "+" + o.level; if (!(k in mins) || o.price < mins[k]) mins[k] = o.price; var dk = today_key(); var dd = market_daily[k] || (market_daily[k] = {}); if (!(dk in dd) || o.price < dd[dk]) dd[dk] = o.price; }
        }
    });
    ...
    try { var cutoff = Date.now() - 8 * 86400000; for (var dk2 in market_daily) { for (var day2 in market_daily[dk2]) { if (new Date(day2).getTime() < cutoff) delete market_daily[dk2][day2]; } if (!Object.keys(market_daily[dk2]).length) delete market_daily[dk2]; } localStorage.setItem("lp_market_daily", JSON.stringify(market_daily)); } catch (e) {}
...
var market_daily = {}; try { market_daily = JSON.parse(localStorage.getItem("lp_market_daily") || "{}"); } catch (e) {} // Item+Lv -> { Tag: Tiefstpreis } (7 Tage)
function market_avg_days(name, level, days) { // Median der Tages-Tiefstpreise der letzten N Tage (0 = keine Daten)
    var dd = market_daily[name + "+" + (level || 0)]; if (!dd) return 0; var cutoff = Date.now() - (days || 3) * 86400000;
    var v = Object.keys(dd).filter(function (d) { return new Date(d).getTime() >= cutoff; }).map(function (d) { return dd[d]; }).sort(function (a, b) { return a - b; });
...
// ---------- Bank-Fenster: Bankinhalt mit NPC-/Marktwert, ausstellen oder beim NPC verkaufen ----------
var bank_rendered = 0;
var bank_panel = null, bank_ui = { sort: "ratio", dir: -1, q: "", f: "" }; try { var bu0 = JSON.parse(localStorage.getItem("lp_bank_ui") || "null"); if (bu0) { for (var bu1 in bu0) bank_ui[bu1] = bu0[bu1]; bank_ui.q = ""; } } catch (e) {}
...
function bank_protect_reason(name) { if (NEVER_SELL.test(name)) return "geschützt (Token-Set / Attribut-Schmuck)"; if (is_team_wish(name)) return "Team-Wunsch (Priest/Ranger)"; if (on_wishlist(name)) return "dein Zielbau"; return ""; }
function bank_rows() {
    ...
    bank_snapshot_items().forEach(function (e) {
        ...
        out.push({ key: e.key, name: e.name, level: e.level, label: nm, q: e.q, n: e.n, stack: !!d.s, npc: npc, mk: mk, avg: avg, ratio: npc > 0 && best > 0 ? best / npc : (best > 0 ? 99 : 0), sug: stand_suggest_price(e.name, e.level), prot: bank_protect_reason(e.name), equip: equip });
    });
...
function toggle_bank_panel() {
    var on = !(bank_panel && bank_panel.parentNode);
    if (on) { bank_panel = init_bank_panel(); render_bank(); } else { bank_panel.remove(); bank_panel = null; }
    try { localStorage.setItem("lp_bank_open", on ? "1" : "0"); } catch (e) {}
}
...
function bank_table_html() {
    ...
    if (!rows.length) h += "<tr><td class='l' colspan='9' style='color:#9aa3b2'>" + (bank_snapshot_items().length ? "nichts passt zum Filter" : "kein Bankstand – beim nächsten Bankgang wird er eingelesen") + "</td></tr>";
    rows.forEach(function (r) {
        ...
        var act = inStand ? "<span class='lp_k'>im Stand-Auftrag</span> <button data-act='standdel' data-key='" + esc(r.key) + "' title='Stand-Auftrag löschen'>✕</button>" : inNpc ? "<span class='lp_k'>NPC-Auftrag offen</span> <button data-act='bnpcdel' data-key='" + esc(r.key) + "' title='NPC-Auftrag löschen'>✕</button>"
            : "<button data-act='bstand' data-key='" + esc(r.key) + "' data-name='" + esc(r.name) + "' data-lvl='" + r.level + "'" + (r.prot && r.prot.indexOf("geschützt") == 0 ? " disabled title='" + esc(r.prot) + "'" : " title='Händler holt es aus der Bank und stellt es zu diesem Preis aus'") + ">Ausstellen</button> <button data-act='bnpc' data-key='" + esc(r.key) + "' data-name='" + esc(r.name) + "' data-lvl='" + r.level + "'" + (r.prot ? " disabled title='" + esc(r.prot) + "'" : " title='Händler holt es aus der Bank und verkauft es beim NPC (Gold in die Händlerkasse)'") + ">NPC</button>";
        h += "<tr class='" + cls + "' title='" + esc(r.name + (r.level ? "+" + r.level : "") + (r.mk ? " · Markt jetzt bei " + r.mk.seller + " (" + pretty_server(r.mk.server) + ")" : "") + (r.prot ? " · " + r.prot : "")) + "'><td class='l'>" + esc(r.label) + (r.level ? " <b>+" + r.level + "</b>" : "") + (r.prot ? " <span class='lp_k' style='font-size:10px'>🔒</span>" : "") + "</td><td>" + (r.stack ? r.q : r.n) + "</td><td>" + qin + "</td><td>" + fmt(Math.round(r.npc)) + "</td><td>" + (r.mk ? fmt(r.mk.price) : "–") + "</td><td>" + (r.avg ? fmt(r.avg) : "–") + "</td><td style='color:" + (r.ratio >= 1.5 ? "#7ed67e" : r.ratio > 0 && r.ratio <= 1.1 ? "#8b93a0" : "#e6e6e6") + "'>" + (r.ratio > 0 && r.ratio < 99 ? "×" + (Math.round(r.ratio * 10) / 10) : "–") + "</td><td>" + pin + "</td><td>" + act + "</td></tr>";
    });
    return h + "</table>";
}
function bank_click(b) { // Klicks im Bank-Fenster (true = verarbeitet)
    ...
    if (act == "bstand") { if (!(price > 0)) { game_log("Bank: Preis fehlt"); return true; } stand_orders[key] = { name: nm, level: lv, q: q, price: price, fixed: price, since: Date.now(), t: Date.now() }; save_stand_orders(); game_log("Stand-Auftrag: " + nm + (lv ? "+" + lv : "") + (q > 1 ? " ×" + q : "") + " für " + fmt(price) + " – Händler holt es aus der Bank und stellt es aus"); last_panel = 0; render_bank(); return true; }
    if (act == "bnpc") { if (bank_protect_reason(nm)) { game_log("Bank: " + nm + " ist " + bank_protect_reason(nm) + " – kein NPC-Verkauf"); return true; } npc_orders[key] = { name: nm, level: lv, q: q, t: Date.now() }; npc_orders_save(); game_log("NPC-Auftrag: " + q + "× " + nm + (lv ? "+" + lv : "") + " – Händler holt es aus der Bank und verkauft es beim NPC (~" + fmt(Math.round(npc_sell_price(nm, lv) * q)) + ")"); render_bank(); return true; }
...
function mkt_action_html(o, i) {
    if (o.b) { var have = inv_count(o.name, o.level), viam2 = merch_can_buy() && !is_pvp_server(o.server); var why = !have ? "nicht in deinem Inventar" : is_pvp_server(o.server) ? "PVP-Server" : (!o.same && !viam2) ? "anderer Server – nur über den Händler möglich (" + (merch_buy ? "Einkauf/Verkauf läuft" : "Händler aus") + ")" : ""; var dqs = G.items[o.name] || {}, qin2 = dqs.s && have > 1 ? "<input data-mq='" + i + "' value='" + Math.min(have, o.q || 1) + "' title='Menge (du hast " + have + ", gesucht " + (o.q || 1) + ")' style='width:38px;font-size:11px;background:#1c2029;color:#eee;border:1px solid #555;margin-right:3px'> " : ""; return qin2 + "<button data-act='msell' data-i='" + i + "' title='" + esc(why || (viam2 ? "Händler holt " + Math.min(have, o.q || 1) + "× " + o.name + " bei dir ab" + (o.same ? "" : ", reist nach " + pretty_server(o.server)) + " und verkauft für " + fmt(o.price) + " je Stück" : "Magier läuft zum Käufer und verkauft " + Math.min(have, o.q || 1) + "× " + o.name)) + "'" + (why ? " disabled" : "") + ">Verkaufen" + (have ? " (" + Math.min(have, o.q || 1) + ")" : "") + (viam2 || !have ? "" : " (Magier)") + "</button>"; }
    var dq = G.items[o.name] || {}, qin = dq.s && (o.q || 1) > 1 ? "<input data-mq='" + i + "' value='" + (o.q || 1) + "' title='Menge (max. " + (o.q || 1) + ")' style='width:38px;font-size:11px;background:#1c2029;color:#eee;border:1px solid #555;margin-right:3px'> " : "";
    if (o.same) { var viam = merch_can_buy() && MB_SAFE_MAPS[o.map]; return qin + "<button data-act='mbuy' data-i='" + i + "' title='" + (viam ? "Händler holt das Gold beim Magier, kauft und bringt es – Magier farmt weiter" : merch_buy ? "Einkauf läuft schon – Magier kauft selbst" : !team_on.merch || !team_running(TEAM.merch) ? "Händler aus – Magier kauft selbst" : "Verkäufer steht auf " + (o.map || "?") + " (für den Händler zu gefährlich) – Magier kauft selbst") + "'>Kaufen" + (viam ? "" : " (Magier)") + "</button> <button data-act='mgoto' data-i='" + i + "' title='nur hinlaufen und pausieren – du schaust selbst'>Hin</button>"; }
    if (is_pvp_server(o.server)) return "<span class='lp_k' title='PVP-Server – kein Serverwechsel'>PVP</span>";
    var viat = merch_can_buy();
    var h = qin + "<button data-act='mtrip' data-i='" + i + "' title='" + (viat ? "Händler holt das Gold, reist auf den Server, kauft und bringt es – Magier farmt weiter" : "Magier wechselt selbst auf diesen Server, kauft und kommt zurück") + " (" + (o.price > character.gold ? "zu wenig Gold" : "Gold reicht") + ")'" + (server_trip || arb_job || merch_buy || o.price > character.gold ? " disabled" : "") + ">Serverkauf" + (viat ? "" : " (Magier)") + "</button>";
...
function market_click(b) { // Klicks im Markt-Fenster (true = verarbeitet)
    ...
    if (mg && !act) { if (b.tagName == "INPUT") return true; mkt.open[mg] = !mkt.open[mg]; render_market(); return true; }
    ...
    var mq = null; try { var qe = b.parentNode && b.parentNode.querySelector("input[data-mq='" + i + "']"); if (qe) { mq = parseInt(qe.value); if (!(mq > 0)) mq = null; } } catch (e) {}
    if (o && mq) o = Object.assign({}, o, { want: mq });
...
async function run_market_job() { // Klick im Markt-Fenster: hinlaufen und kaufen bzw. nur hinlaufen
    ...
    try {
        ...
        if (j.mode == "sell") {
            ...
            if (lo) {
                var d = G.items[o.name] || {}, price = lo.it.price, q = d.s ? Math.max(1, Math.min(o.want || lo.it.q || 1, Math.floor(spendable() / price))) : 1;
...
var MG = { target: 5000000, min: 1000000, esc_max: 2000000, mat_max: 20000 }; try { var mg0 = JSON.parse(localStorage.getItem("lp_merch_gold") || "null"); if (mg0) for (var mgk in mg0) MG[mgk] = mg0[mgk]; if (MG.esc_max < 2000000 && localStorage.getItem("lp_mg_v264") != "1") { MG.esc_max = 2000000; localStorage.setItem("lp_mg_v264", "1"); localStorage.setItem("lp_merch_gold", JSON.stringify(MG)); } } catch (e) {} // Händler-Kasse: Ziel, Nachfüllgrenze, Obergrenze Priest/Ranger
...
function merch_buy_start(o, mode) {
    ...
    var d = G.items[o.name] || {}, q = d.s ? Math.max(1, Math.min(o.want || o.q || 1, Math.floor(budget / o.price))) : 1;
...
function merch_sell_start(o) {
    ...
    var q = Math.max(1, Math.min(o.want || o.q || 1, have));
...
function merch_buy_tick() {
    ...
    if (j.stage == "away") { if (!arb_job) { if (j.for_merch) { delete merch_needs[j.o.name]; merch_needs_save(); mb_done(j.o.name + " – Reise beendet, bleibt beim Händler"); return; } if (j.kind == "sell") { mb_done("Verkaufsreise beendet – Erlös liegt in der Händlerkasse"); return; } j.stage = "fetch"; j.t = Date.now(); j.tries = 0; mb_save(); } else if (el > 20 * 60000) mb_fail("Reise ohne Ende"); return; }
}
function mb_start_trip() { // Handelsreise mit genau diesem Angebot, Item wird behalten
    ...
    var nm = TEAM.merch;
    arb_job = { id: Date.now(), server: o.server, region: sp.region, sid: sp.id, offers: [{ name: o.name, level: o.level, price: o.price, q: j.q, seller: o.seller, map: o.map, x: o.x, y: o.y, tslot: o.tslot, profit: 0, sell: j.kind == "sell", for_merch: !!j.for_merch }], t: Date.now(), stage: "stopped", profit: 0, keep: true, sellonly: j.kind == "sell", buy_id: j.id };
    arb_save_job(); try { localStorage.removeItem("lp_arb_result_" + nm); } catch (e) {}
    last_arb_trip = Date.now();
    try { if (active_chars()[nm]) stop_character(nm); } catch (e) {}
    try { parent.__lp_team_restart_after = Date.now() + 12 * 60000; } catch (e) {}
...
async function scan_all_merchants(force, only_slot) {
    ...
    merchants.forEach(function (m) {
        for (var sl in m.slots) {
            ...
            if (it.b) continue;
            if (arb_bad({ name: it.name, level: it.level || 0, seller: m.name, price: it.price })) continue; // Händler konnte es nicht kaufen (Schein-Angebot)
...
async function run_pending_buy() { // zum Händler auf diesem Server laufen und kaufen
    ...
    try {
        ...
        if (lo) { var seller = lo.seller, it = lo.it, dq = G.items[it.name] || {}, qn = f.market && dq.s ? Math.max(1, Math.min(f.want || it.q || 1, Math.floor(spendable() / it.price))) : 1; await wish_buy_from_offer({ name: it.name, level: it.level || 0, price: it.price, force: !!f.force, slot: f.slot, seller: f.seller }, async function () { trade_buy(seller, lo.slot, qn); return true; }); }
...
async function upgrade_inv(name, level, target, stat_want) { // stat_want: Attribut, das vor dem Sprung auf Qualität 1 gesetzt wird
    while (level < target) {
        check_pause(); if (build_running && build_stop) return { level: level, destroyed: false, stopped: true };
...
// ---------- Bauauftrag: ein NPC-kaufbares Teil in Stückzahl auf Zielstufe bauen – ohne Zerstörungszähler, nur Stückzahl / Gold-Reserve / Stop ----------
var build_order = null; try { build_order = JSON.parse(localStorage.getItem("lp_build_" + character.name) || "null"); } catch (e) {}
...
function build_start() {
    ...
    var name = build_ui.item, def = G.items[name]; if (!def || !is_buyable(name)) { game_log("Bauauftrag: " + name + " ist nicht beim NPC kaufbar"); return; }
    var level = Math.max(1, Math.min(parseInt(build_ui.level) || 0, max_level(def))), n = Math.max(1, parseInt(build_ui.n) || 1);
    if (!build_order || build_order.name != name || build_order.level != level || build_order.done >= build_order.n) build_order = { name: name, level: level, n: n, done: 0, destroyed: 0, spent: 0, reserve: build_ui.reserve || 0 };
    else { build_order.n = n; build_order.reserve = build_ui.reserve || 0; }
    build_save(); preempt("Bauauftrag", build_run);
}
async function build_run() {
    var o = build_order; if (!o || build_running) return;
    if (busy || routine_running()) { game_log("Bauauftrag: gerade beschäftigt – gleich nochmal"); return; }
    unpause("Bauauftrag"); build_running = true; build_stop = false; upgrading = true; busy = true; last_panel = 0;
    var saved_reserve = GOLD_RESERVE; GOLD_RESERVE = Math.max(GOLD_RESERVE, o.reserve || WISH_RESERVE);
    var def = G.items[o.name], comp = !!def.compound;
    game_log("Bauauftrag: " + o.name + " +" + o.level + " × " + o.n + " (" + o.done + " fertig) – läuft bis Stückzahl, Reserve " + fmt(GOLD_RESERVE) + " oder Stop");
    try {
        await travel_place(comp ? "compound" : "upgrade");
        while (o.done < o.n) {
            check_pause(); if (build_stop) throw "STOP";
            busy_since = Date.now(); // Wächter nicht auslösen – der Auftrag darf lange laufen
            if (character.esize < 4) { game_log("Bauauftrag: Inventar voll (frei " + character.esize + ") – Stop"); break; }
            var g0 = character.gold, r = comp ? await build_compound_step(o) : await build_upgrade_step(o);
            o.spent += Math.max(0, g0 - character.gold);
            if (r == "done") { o.done++; game_log("Bauauftrag: " + o.name + " +" + o.level + " fertig (" + o.done + "/" + o.n + ", bisher " + o.destroyed + " zerstört, " + fmt(o.spent) + " Gold)"); }
            build_save(); last_panel = 0;
            if (r == "stop") break;
        }
    } catch (e) { game_log(e == "PAUSE" ? "Bauauftrag: durch Pause unterbrochen – Start setzt fort" : e == "STOP" ? "Bauauftrag: gestoppt (" + o.done + "/" + o.n + ")" : e == "ABORT" ? "Bauauftrag: abgebrochen" : "Bauauftrag-Fehler: " + err_txt(e)); }
    GOLD_RESERVE = saved_reserve; build_save();
    if (o.done >= o.n) game_log("Bauauftrag fertig: " + o.n + "× " + o.name + " +" + o.level + " im Inventar · " + o.destroyed + " zerstört · " + fmt(o.spent) + " Gold");
    build_running = false; build_step_txt = ""; upgrading = false; busy = false; last_panel = 0;
    after_action("Bauauftrag");
}
async function build_upgrade_step(o) { // eine Kopie unter Zielstufe (höchste zuerst) hochziehen; keine da: eine kaufen
    var idx = -1, lv = -1;
    for (var i = 0; i < character.items.length; i++) { var it = character.items[i]; if (it && it.name == o.name && (it.level || 0) < o.level && (it.level || 0) > lv) { idx = i; lv = it.level || 0; } }
    var def = G.items[o.name];
    if (idx < 0) {
        if (spendable() < def.g) { game_log("Bauauftrag: Reserve erreicht (" + fmt(character.gold) + ") – Stop"); return "stop"; }
        build_step_txt = "kaufe " + o.name; if (!await buy_items(o.name, 1)) return "stop";
        await travel_place("upgrade"); lv = 0;
    }
    build_step_txt = "+" + lv + " → +" + o.level;
    var r = await upgrade_inv(o.name, lv, o.level, null);
    if (r.destroyed) { o.destroyed++; game_log("Bauauftrag: " + o.name + " bei +" + r.level + " zerstört (" + o.destroyed + ". Mal) – nächstes Teil"); return "again"; }
    if (r.stopped) return "stop";
    return r.level >= o.level ? "done" : "again";
}
async function build_compound_step(o) { // Schmuck: unterste Stufe mit 3 Kopien compounden; keine: +0 nachkaufen
    var def = G.items[o.name], t = o.level, n_t0 = find_inv_indices(o.name, t).length, l = -1;
    for (var k = t - 1; k >= 0; k--) if (find_inv_indices(o.name, k).length >= 3) { l = k; break; }
    if (l < 0) {
        var need = 3 - find_inv_indices(o.name, 0).length;
        if (spendable() < def.g * need) { game_log("Bauauftrag: Reserve erreicht (" + fmt(character.gold) + ") – Stop"); return "stop"; }
        build_step_txt = "kaufe " + need + "× " + o.name; if (!await buy_items(o.name, need)) return "stop";
        await travel_place("compound"); return "again";
    }
    var scroll = "cscroll" + best_scroll_grade(def, l, "c", cost_to_reach(o.name, l));
    if (spendable() < G.items[scroll].g) { game_log("Bauauftrag: Reserve erreicht – Stop"); return "stop"; }
    if (quantity(scroll) < 1) { buy(scroll, 1); await sleep(600); }
    var sc = locate_item(scroll); if (sc < 0) { game_log("Bauauftrag: keine " + scroll); return "stop"; }
    var idx = find_inv_indices(o.name, l), prev = find_inv_indices(o.name, l + 1).length;
...
function start_main() {
  ...
  main_timer = parent.__lp_main_timer = setInterval(function () {
    ...
    if (event_mode == "abtesting") { if (character.map == "abtesting") ab_step(); else set_message("PvP: warte auf Sprung"); return; }
    ...
    var hold = spot_needs_team(farm) && !!beh; // Team hängt zurück: kein neues Ziel, nur Verteidigung (Schonfrist nach Ankunft: kein Sammelpunkt/Rückzug/Entgegengehen – aber auch kein Angriff ohne Team)
    ...
    if (!target && hold) { // Team hängt zurück: nur Angreifer auf mich abwehren, sonst stehen bleiben
        ...
        if (!target) { set_message(wait_txt()); if (team_grace()) return; wait_log(wait_txt() + " – kämpfe nicht allein bei " + farm); if (!rally_back(farm)) meet_escort(); return; }
...
try { if (localStorage.getItem("lp_bank_open") == "1") { bank_panel = init_bank_panel(); render_bank(); } } catch (e) {}
start_main();
